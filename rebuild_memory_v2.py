...
if not incoming_tested.empty:
    ...
    df_verify = pd.read_parquet(data_root / "artifacts" / "experiments" / program_id / "memory" / "tested_regions.parquet")
    print("Verified from file immediately after write:")
    print(df_verify[['candidate_id', 'mean_return_bps']].head())
    
    write_memory_table(program_id, "region_statistics", compute_region_statistics(incoming_tested), data_root=data_root)
    write_memory_table(program_id, "event_statistics", compute_event_statistics(incoming_tested), data_root=data_root)
    write_memory_table(program_id, "template_statistics", compute_template_statistics(incoming_tested), data_root=data_root)
    write_memory_table(program_id, "context_statistics", compute_context_statistics(incoming_tested), data_root=data_root)
