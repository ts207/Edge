...
_SCHEMA_REGISTRY: Dict[str, DataFrameSchemaContract] = {
    ...
    'promotion_audit': DataFrameSchemaContract(
        ...
        optional_columns=('q_value', 'promotion_score', 'bundle_version', 'policy_version', 'evidence_bundle_json'),
    ),
...
def normalize_dataframe_for_schema(df: pd.DataFrame, schema_name: str) -> pd.DataFrame:
    ...
    for col in schema.required_columns + schema.optional_columns:
        if col not in out.columns:
            out[col] = pd.NA
    ordered = list(schema.required_columns) + [c for c in schema.optional_columns if c in out.columns]
    remainder = [c for c in out.columns if c not in ordered]
    return out[ordered + remainder]


def validate_dataframe_for_schema(df: pd.DataFrame, schema_name: str, *, allow_empty: bool = True) -> pd.DataFrame:
    schema = get_schema_contract(schema_name)
    ...
    if missing:
        raise ValueError(f"dataframe for schema '{schema_name}' missing required columns: {missing}")
    ...
    if not out.empty:
        ...
        if bad:
            raise ValueError(f"dataframe for schema '{schema_name}' has all-null required columns: {bad}")
    return out
