...
STAGE_ARTIFACT_REGISTRY: tuple[StageArtifactContract, ...] = (
    StageArtifactContract(stage_patterns=("ingest_binance_um_funding",), outputs=("raw.perp.funding_5m",)),
    StageArtifactContract(stage_patterns=("ingest_binance_um_liquidation_snapshot",), outputs=("raw.perp.liquidations",)),
    StageArtifactContract(stage_patterns=("ingest_binance_um_open_interest_hist",), outputs=("raw.perp.open_interest",)),
    StageArtifactContract(stage_patterns=("ingest_binance_um_ohlcv_{tf}",), outputs=("raw.perp.ohlcv_{tf}",)),
    StageArtifactContract(stage_patterns=("ingest_binance_spot_ohlcv_{tf}",), outputs=("raw.spot.ohlcv_{tf}",)),
    StageArtifactContract(stage_patterns=("build_cleaned_{tf}",), inputs=("raw.perp.ohlcv_{tf}",), outputs=("clean.perp.*",), external_inputs=("raw.perp.ohlcv_{tf}",)),
    StageArtifactContract(stage_patterns=("build_cleaned_{tf}_spot",), inputs=("raw.spot.ohlcv_{tf}",), outputs=("clean.spot.*",), external_inputs=("raw.spot.ohlcv_{tf}",)),
    StageArtifactContract(
        stage_patterns=("build_features_{tf}",),
        inputs=("clean.perp.*",),
        optional_inputs=("raw.perp.funding_{tf}", "raw.perp.liquidations", "raw.perp.open_interest"),
        outputs=("features.perp.v2",),
        external_inputs=("clean.perp.*", "raw.perp.funding_{tf}", "raw.perp.liquidations", "raw.perp.open_interest")
    ),
    StageArtifactContract(stage_patterns=("build_features_{tf}_spot",), inputs=("clean.spot.*",), outputs=("features.spot.v2",), external_inputs=("clean.spot.*",)),
    StageArtifactContract(stage_patterns=("build_universe_snapshots",), inputs=("clean.perp.*",), outputs=("metadata.universe_snapshots",), external_inputs=("clean.perp.*",)),
    StageArtifactContract(stage_patterns=("build_context_features*",), inputs=("features.perp.v2",), outputs=("context.features",), external_inputs=("features.perp.v2",)),
    StageArtifactContract(stage_patterns=("build_market_context*",), inputs=("features.perp.v2",), outputs=("context.market_state",), external_inputs=("features.perp.v2",)),
    StageArtifactContract(stage_patterns=("build_microstructure_rollup*",), inputs=("features.perp.v2",), outputs=("context.microstructure",), external_inputs=("features.perp.v2",)),
    StageArtifactContract(stage_patterns=("validate_feature_integrity*",), inputs=("features.perp.v2",)),
    StageArtifactContract(stage_patterns=("validate_data_coverage*",), inputs=("clean.perp.*",)),
    StageArtifactContract(stage_patterns=("validate_context_entropy",), inputs=("context.features",)),
    StageArtifactContract(stage_patterns=("build_normalized_replay_stream",), inputs=("metadata.universe_snapshots",), outputs=("runtime.normalized_stream",)),
    StageArtifactContract(stage_patterns=("run_causal_lane_ticks",), inputs=("runtime.normalized_stream",), outputs=("runtime.causal_ticks",)),
    StageArtifactContract(stage_patterns=("run_determinism_replay_checks",), inputs=("runtime.causal_ticks",), outputs=("runtime.determinism_checks",)),
    StageArtifactContract(stage_patterns=("run_oms_replay_validation",), inputs=("runtime.causal_ticks",), outputs=("runtime.oms_replay",)),
    StageArtifactContract(stage_patterns=("analyze_*",), optional_inputs=("features.perp.v2", "context.market_state", "context.microstructure"), outputs=("phase1.events.{event_type}",), external_inputs=("features.perp.v2", "context.market_state", "context.microstructure")),
    StageArtifactContract(stage_patterns=("phase1_correlation_clustering",), optional_inputs=("phase1.events.*",), outputs=("phase1.correlation_clustering",)),
    StageArtifactContract(stage_patterns=("build_event_registry*",), optional_inputs=("phase1.events.*",), outputs=("phase2.event_registry.{event_type}",), external_inputs=("phase1.events.*",)),
    StageArtifactContract(stage_patterns=("canonicalize_event_episodes*",), inputs=("phase2.event_registry.{event_type}",), outputs=("phase2.event_episodes.{event_type}",), external_inputs=("phase2.event_registry.{event_type}",)),
    StageArtifactContract(stage_patterns=("phase2_conditional_hypotheses*",), inputs=("phase2.event_episodes.{event_type}", "features.perp.v2"), optional_inputs=("context.market_state", "context.microstructure"), outputs=("phase2.candidates.{event_type}",), external_inputs=("phase2.event_episodes.{event_type}", "features.perp.v2", "context.market_state", "context.microstructure")),
    StageArtifactContract(stage_patterns=("bridge_evaluate_phase2*",), inputs=("phase2.candidates.{event_type}",), outputs=("phase2.bridge_metrics.{event_type}", "phase2.bridge_summary.{event_type}", "phase2.bridge_enriched_candidates.{event_type}"), external_inputs=("phase2.candidates.{event_type}",)),
    StageArtifactContract(stage_patterns=("phase2_search_engine",), inputs=("features.perp.v2",), outputs=("phase2.candidates.search",), external_inputs=("features.perp.v2",)),
    StageArtifactContract(stage_patterns=("finalize_experiment",), optional_inputs=("phase2.candidates.*",), outputs=("experiment.tested_ledger",)),
    StageArtifactContract(stage_patterns=("summarize_discovery_quality",), inputs=("phase2.candidates.*",), optional_inputs=("phase2.bridge_summary.*",), outputs=("phase2.discovery_quality_summary",)),
    StageArtifactContract(stage_patterns=("evaluate_naive_entry",), inputs=("phase2.candidates.*",), outputs=("phase2.naive_entry_eval",)),
    StageArtifactContract(stage_patterns=("export_edge_candidates",), inputs=("phase2.candidates.*",), optional_inputs=("phase2.bridge_metrics.*",), outputs=("edge_candidates.normalized",), external_inputs=("phase2.candidates.*", "phase2.bridge_metrics.*")),
    StageArtifactContract(stage_patterns=("promote_candidates",), inputs=("edge_candidates.normalized",), optional_inputs=("phase2.bridge_metrics.*", "phase2.naive_entry_eval"), outputs=("promotion.audit", "promotion.promoted_candidates"), external_inputs=("edge_candidates.normalized", "phase2.bridge_metrics.*", "phase2.naive_entry_eval")),
    StageArtifactContract(stage_patterns=("update_edge_registry",), inputs=("promotion.audit", "promotion.promoted_candidates"), outputs=("history.candidate.edge_observations", "history.candidate.edge_registry", "edge_registry.snapshot"), external_inputs=("promotion.audit", "promotion.promoted_candidates")),
    StageArtifactContract(stage_patterns=("update_campaign_memory",), optional_inputs=("edge_candidates.normalized", "promotion.audit", "history.candidate.edge_registry", "phase2.discovery_quality_summary"), outputs=("experiment.memory.tested_regions", "experiment.memory.reflections", "experiment.memory.failures"), external_inputs=("edge_candidates.normalized", "promotion.audit", "history.candidate.edge_registry", "phase2.discovery_quality_summary")),
    StageArtifactContract(stage_patterns=("analyze_conditional_expectancy",), inputs=("history.candidate.edge_registry",), outputs=("research.expectancy_analysis",), external_inputs=("history.candidate.edge_registry",)),
    StageArtifactContract(stage_patterns=("validate_expectancy_traps",), inputs=("research.expectancy_analysis",), outputs=("research.expectancy_traps",)),
    StageArtifactContract(stage_patterns=("generate_recommendations_checklist",), inputs=("research.expectancy_traps",), outputs=("research.recommendations_checklist",)),
    StageArtifactContract(stage_patterns=("compile_strategy_blueprints",), inputs=("research.recommendations_checklist",), outputs=("strategy.blueprints",)),
    StageArtifactContract(stage_patterns=("build_strategy_candidates",), inputs=("strategy.blueprints",), outputs=("strategy.candidates",)),
    StageArtifactContract(stage_patterns=("select_profitable_strategies",), optional_inputs=("strategy.candidates",), outputs=("strategy.profitable",), external_inputs=("strategy.candidates",)),
)
...
def resolve_stage_artifact_contract(
    ...
    return (
        ResolvedStageArtifactContract(
            inputs=_materialize_artifact_keys(contract.inputs, stage_name=stage_name, base_args=base_args),
            optional_inputs=_materialize_artifact_keys(contract.optional_inputs, stage_name=stage_name, base_args=base_args),
            outputs=_materialize_artifact_keys(contract.outputs, stage_name=stage_name, base_args=base_args),
            external_inputs=_materialize_artifact_keys(contract.external_inputs, stage_name=stage_name, base_args=base_args),
        ),
        issues,
    )
