...
from project.strategies.dsl_runtime.signal_resolution import compute_trigger_coverage
from project.strategies.dsl_runtime.evaluator import entry_eligibility_mask
    ...
    for i in range(n):
        ...
        if in_pos != 0:
            ...
            if break_even_r > 0 and not np.isnan(risk_per_unit) and risk_per_unit > 0:
                if in_pos > 0:
                    if (best_price - entry_price) / risk_per_unit >= break_even_r: stop_price = max(stop_price, entry_price)
                else:
                    if (entry_price - best_price) / risk_per_unit >= break_even_r: stop_price = min(stop_price, entry_price)

            if trailing_stop_type != 0:
                trail = trailing_offsets[i]
                if trail > 0:
                    if in_pos > 0: stop_price = max(stop_price, best_price - trail)
                    else: stop_price = min(stop_price, best_price + trail)

            should_exit = False; code = 0
            if held >= time_stop_bars: should_exit = True; code = 2
            elif invalidate: should_exit = True; code = 3
                ...
                if hit_stop and hit_target:
                    if priority_randomisation and random_rolls[i] > 0.5: should_exit = True; code = 5
                    else: should_exit = True; code = 4
                elif hit_stop: should_exit = True; code = 4
                elif hit_target: should_exit = True; code = 5

            if should_exit:
                in_pos = 0; state = 3
                cooldown_until = i + max(cooldown_bars, reentry_lockout_bars)
                events_timestamps.append(i); events_codes.append(code)

        if in_pos == 0:
            if state == 3 and i < cooldown_until:
                positions[i] = 0; continue
            if state == 3 and i >= cooldown_until: state = 0
            if state == 0:
                if eligible[i]: state = 1; arm_remaining = arm_bars_base; positions[i] = 0; continue
                else: positions[i] = 0; continue
            if state == 1:
                if arm_remaining > 0:
                    arm_remaining -= 1
                    if arm_remaining > 0: positions[i] = 0; continue
                side = entry_sides[i]
                if side == 0 or c <= 0: state = 0; positions[i] = 0; continue
                stop_off = stop_offsets[i]; target_off = target_offsets[i]
                if stop_off <= 0 or target_off <= 0: state = 0; positions[i] = 0; continue
                in_pos = side; state = 2; entry_idx = i; entry_price = open_prices[i]
                risk_per_unit = stop_off; best_price = open_prices[i]
                if side > 0: stop_price = entry_price - stop_off; target_price = entry_price + target_off
                else: stop_price = entry_price + stop_off; target_price = entry_price - target_off
                events_timestamps.append(i); events_codes.append(1)
        positions[i] = in_pos
...
def _vectorized_offset(frame: pd.DataFrame, kind: str, value: float) -> np.ndarray:
    ...
    valid = close > 0
    val = float(value)
    if kind == "percent": out[valid] = close[valid] * val
    elif kind == "range_pct":
        ...
        atr = _to_float_arr(frame.get("atr_14", pd.Series(np.nan, index=frame.index)).values, np.nan)
...
class DslInterpreterV1:
    ...
    def generate_positions(self, bars: pd.DataFrame, features: pd.DataFrame, params: dict) -> pd.Series:
        ...
        if raw_blueprint is None and raw_executable is None:
            raise ValueError("dsl_interpreter_v1 requires params.dsl_blueprint or params.executable_strategy_spec")
        ...
        bars = bars.copy(); features = features.copy()
        bars["timestamp"] = pd.to_datetime(bars["timestamp"], utc=True)
        features["timestamp"] = pd.to_datetime(features["timestamp"], utc=True)
        merged = features.drop(columns=["open","high","low","close", "volume", "quote_volume"], errors="ignore").merge(
            bars[["timestamp","open","high","low","close", "volume", "quote_volume"]], on="timestamp", how="left"
        )
        ...
        open_arr = _to_float_arr(frame["open"].values); close_arr = _to_float_arr(frame["close"].values)
        high_arr = _to_float_arr(frame["high"].values); low_arr = _to_float_arr(frame["low"].values)
        ...
        if inv_col in frame.columns:
            m_arr = _to_float_arr(frame[inv_col].values); val = float(inv.get("value", 0))
            op = str(inv.get("operator", ""))
            if op == ">": invalidation_arr = m_arr > val
            elif op == "<": invalidation_arr = m_arr < val

        entry_sides = np.zeros(len(frame), dtype=np.int32)
        bias = event_direction_bias(blueprint.event_type)
        if blueprint.direction == "long": entry_sides[:] = 1
        elif blueprint.direction == "short": entry_sides[:] = -1
        ...
        pos_arr, ev_idxs, ev_codes = generate_positions_numba(
            frame["timestamp"].values.astype(np.int64), open_arr, close_arr, high_arr, low_arr,
            eligible_arr, entry_sides, stop_offsets, target_offsets,
            bool(params.get("allow_intrabar_exits", False)), int(blueprint.entry.cooldown_bars),
            int(blueprint.entry.reentry_lockout_bars), max(1, int(blueprint.entry.delay_bars)),
            int(blueprint.exit.time_stop_bars), float(blueprint.exit.break_even_r), 0, np.zeros(len(frame)),
            invalidation_arr, bool(params.get("priority_randomisation", False)), random_rolls
        )
