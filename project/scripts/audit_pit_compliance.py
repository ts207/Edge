...
def audit_features():
    ...
    def wrap_rv(d): return calculate_rv_percentile_24h(d["close"])
    ...
    def wrap_lv(d): return detect_liquidity_vacuum_events(d, "TEST", cfg=cfg)
    res_lv = verify_pit_compliance(wrap_lv, df)
    print(f"Result: {'PASS' if res_lv['is_compliant'] else 'FAIL'}")
    if not res_lv['is_compliant']:
        print(f"  Leak detected! Future data affects past event evaluation via global thresholding.")
