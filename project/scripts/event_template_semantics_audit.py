    ...
    for event_id in registry.event_ids:
        ...
        if raw_templates != runtime_templates_for_event:
            payload = {
                ...
                "legacy_family": runtime_cfg.legacy_family,
            }
...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    if runtime_drops:
        for event_id, payload in runtime_drops.items():
            lines.append(
                f"- `{event_id}`: raw={payload['raw_templates']}, runtime={payload['runtime_templates']}, family=`{payload['family']}`, legacy=`{payload['legacy_family']}`"
            )
    else:
        lines.append("- None")
    lines.extend(
