...
def build_smoke_bars(symbol: str, *, periods: int = 96, freq: str = '5min', seed: int = 0) -> pd.DataFrame:
...
def build_smoke_events(symbol: str, *, periods: int = 48, freq: str = '15min', seed: int = 0) -> pd.DataFrame:
    rng = _rng(seed + abs(hash(symbol)) % 1000)
    ts = pd.date_range('2024-01-01', periods=periods, freq=freq, tz='UTC')
...
def build_smoke_dataset(output_root: Path, *, seed: int = 20260101, storage_mode: str = 'auto') -> SmokeDatasetInfo:
    ...
    for symbol in SMOKE_SYMBOLS:
        ...
        feature_dir = root / 'lake' / 'features' / 'perp' / symbol / '5m' / feature_dataset_dir_name()
        bar_dir = root / 'lake' / 'cleaned' / 'perp' / symbol / 'bars_5m'
        ensure_dir(feature_dir)
        ensure_dir(bar_dir)
        _write_df(bars[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume', 'funding_rate_scaled', 'direction_score']], feature_dir / 'slice.parquet')
        _write_df(bars[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume']], bar_dir / 'slice.parquet')
    ...
    (root / 'runs' / SMOKE_RUN_ID / 'run_manifest.json').write_text(json.dumps(run_manifest, indent=2, sort_keys=True), encoding='utf-8')
    return SmokeDatasetInfo(root=root, run_id=SMOKE_RUN_ID, symbols=SMOKE_SYMBOLS, seed=int(seed), start_ts=start_ts, end_ts=end_ts)


def run_engine_smoke(dataset: SmokeDatasetInfo) -> Dict[str, Any]:
    from project.strategy.dsl.schema import Blueprint, SymbolScopeSpec, EntrySpec, ExitSpec, SizingSpec, LineageSpec, EvaluationSpec

    bp = Blueprint(
        id='smoke_dsl', run_id=dataset.run_id, event_type='mock_event', candidate_id='smoke_candidate',
        direction='long',
        ...
        exit=ExitSpec(time_stop_bars=5, invalidation={'metric': 'smoke', 'operator': '==', 'value': 0.0}, stop_type='percent', stop_value=0.05, target_type='percent', target_value=0.05, trailing_stop_type='none', trailing_stop_value=0.0, break_even_r=0.0),
        ...
        overlays=[], evaluation=EvaluationSpec(min_trades=0, cost_model={'fees_bps': 5.0, 'slippage_bps': 0.0, 'funding_included': True}, robustness_flags={'oos_required': False, 'multiplicity_required': False, 'regime_stability_required': False}), lineage=LineageSpec(source_path='smoke', compiler_version='smoke', generated_at_utc='smoke')
    )
    result = run_engine(
...
def run_research_smoke(dataset: SmokeDatasetInfo) -> Dict[str, Any]:
    ...
    write_candidate_reports(
        ...
        diagnostics={'run_id': dataset.run_id, 'smoke_dataset_version': SMOKE_DATASET_VERSION, 'combined_candidate_rows': int(len(combined))},
    )
    return {'combined_candidates': combined, 'symbol_candidates': symbol_candidates, 'output_dir': out_dir}


def build_smoke_edge_candidates(research_result: Dict[str, Any], dataset: SmokeDatasetInfo) -> pd.DataFrame:
    ...
    for i, rec in enumerate(combined.to_dict(orient='records')):
        ...
        rows.append({
            ...
            'bridge_validation_after_cost_bps': float(rec.get('estimate_bps', 15.0 if promoted_like else -2.0)),
            'bridge_validation_stressed_after_cost_bps': float(rec.get('estimate_bps', 12.0 if promoted_like else -3.0)),
            'net_expectancy_bps': float(rec.get('estimate_bps', 12.0 if promoted_like else -3.0)),
            ...
            'regime_mean_map': {'low': 8.0, 'high': 6.0} if promoted_like else {'low': -1.0, 'high': 0.5},
            'regime_counts': {'low': 40, 'high': 35},
            'symbol_expectancy_map': {symbol: float(rec.get('estimate_bps', 15.0 if promoted_like else -2.0))},
...
def materialize_smoke_promotion_inputs(dataset: SmokeDatasetInfo, research_result: Dict[str, Any]) -> pd.DataFrame:
    ...
    ensure_dir(neg_dir)
    (neg_dir / 'negative_control_summary.json').write_text(json.dumps({'by_event': {SMOKE_EVENT_TYPE: {'pass_rate_after_bh': 0.0}}}, indent=2), encoding='utf-8')
    return candidates


def run_promotion_smoke(dataset: SmokeDatasetInfo, research_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = run_promotion_cli([
        ...
        '--require_multiplicity_diagnostics', '1',
    ])
...
def build_smoke_summary(*, dataset: SmokeDatasetInfo, storage_mode: str) -> Dict[str, Any]:
    config_hash = hashlib.sha256(json.dumps({'seed': dataset.seed, 'storage_mode': storage_mode, 'symbols': list(dataset.symbols)}, sort_keys=True).encode('utf-8')).hexdigest()
    return summarize_manifest_environment(
        git_sha=os.getenv('GITHUB_SHA', 'local'),
        python_version=sys.version.split()[0],
        storage_mode='csv-fallback' if os.getenv('BACKTEST_FORCE_CSV_FALLBACK', '0') in {'1', 'true', 'TRUE'} else 'parquet',
        smoke_dataset_version=SMOKE_DATASET_VERSION,
