...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Run deterministic smoke workflows and artifact validation.')
    parser.add_argument('--mode', choices=['engine', 'research', 'promotion', 'full', 'validate-artifacts'], default='full')
    ...
    try:
        ...
        (root / 'reliability' / 'smoke_failure.json').write_text(json.dumps({'error': str(exc), 'mode': args.mode}, indent=2), encoding='utf-8')
        raise
