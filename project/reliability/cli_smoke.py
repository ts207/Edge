...
def run_smoke_cli(mode: str, *, root: Path, seed: int = 20260101, storage_mode: str = 'auto') -> Dict[str, Any]:
    dataset = None
    ...
    if mode in {'research', 'promotion', 'full'}:
        ...
        candidate_path = _read_first_matching(Path(research_result['output_dir']), 'phase2_candidates')
    ...
    if mode == 'validate-artifacts':
        ...
        promo_dirs = list((root / 'reports' / 'promotions').glob('*')) if (root / 'reports' / 'promotions').exists() else []
        if promo_dirs:
            result['promotion'] = validate_promotion_artifacts(promo_dirs[0])
        engine_roots = list((root / 'reports' / 'backtests').glob('*')) if (root / 'reports' / 'backtests').exists() else []
        if not engine_roots:
    ...
    (root / 'reliability' / 'smoke_summary.json').write_text(json.dumps(summary, indent=2, sort_keys=True), encoding='utf-8')
    return summary
