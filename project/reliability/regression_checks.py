...
def assert_next_open_entry_economics_preserved(close: pd.Series, open_: pd.Series, target_position: pd.Series) -> pd.DataFrame:
    ledger = compute_pnl_ledger(target_position, close, open_=open_, execution_mode='next_open', cost_bps=0.0)
...
def require_full_market_bars(events_df: pd.DataFrame, market_df: pd.DataFrame, *, event_time_col: str = 'timestamp', market_time_col: str = 'timestamp') -> None:
    if events_df.empty or market_df.empty:
        raise AssertionError('events and market data must both be non-empty')
    event_times = pd.to_datetime(events_df[event_time_col], utc=True, errors='coerce').dropna().unique()
    market_times = pd.to_datetime(market_df[market_time_col], utc=True, errors='coerce').dropna().unique()
    ...
    if set(map(str, market_times)) == set(map(str, event_times)):
        raise AssertionError('market proxy matches event timestamps exactly; expected full market bars')
