...
STRATEGY_TRACE_SCHEMA = ArtifactSchemaSpec(
    ...
    required_columns=(
        'timestamp', 'strategy', 'symbol', 'signal_position', 'target_position',
        'executed_position', 'gross_pnl', 'net_pnl',
    ),
    optional_columns=('fill_mode', 'fill_price', 'allocation_scale', 'allocation_clip_reason'),
    non_null_columns=('timestamp', 'strategy', 'symbol', 'signal_position', 'target_position', 'executed_position'),
...
PORTFOLIO_LEDGER_SCHEMA = ArtifactSchemaSpec(
    ...
    required_columns=(
        'timestamp', 'gross_pnl', 'net_pnl', 'equity', 'equity_return', 'gross_exposure', 'net_exposure', 'turnover',
    ),
...
CANDIDATE_TABLE_SCHEMA = ArtifactSchemaSpec(
    ...
    required_columns=(
        'candidate_id', 'event_type', 'symbol', 'run_id', 'split_scheme_id', 'estimate_bps',
        'p_value_raw', 'p_value_adj', 'correction_family_id', 'correction_method',
    ),
    optional_columns=('q_value', 'q_value_by', 'q_value_cluster', 'n_obs', 'n_clusters', 'hypothesis_id'),
...
ENGINE_MANIFEST_SCHEMA = ManifestSchemaSpec(
    ...
    required_keys=('manifest_type', 'manifest_version', 'run_id', 'artifacts', 'schemas', 'metrics'),
...
STAGE_MANIFEST_SCHEMA = ManifestSchemaSpec(
    ...
    required_keys=('run_id', 'stage', 'stage_instance_id', 'status', 'parameters', 'inputs', 'outputs', 'ontology_spec_hash'),
