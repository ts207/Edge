...
from project.reliability.manifest_checks import validate_manifest_artifacts_exist, validate_manifest_core
...
def _normalize_portfolio_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available = {k: v for k, v in rename_map.items() if k in frame.columns and v not in frame.columns}
    if available:
...
def _validate_dataframe(df: pd.DataFrame, spec: ArtifactSchemaSpec) -> pd.DataFrame:
    ...
    for col in spec.monotonic_by:
        if col in frame.columns:
            ...
            if series.isna().any() or not series.is_monotonic_increasing:
                raise ValueError(f"{spec.artifact_type} column {col} must be monotonic increasing timestamps")
    if spec.unique_by:
    ...
    for col, allowed in spec.enum_columns.items():
        if col in frame.columns:
            ...
            if bad:
                raise ValueError(f"{spec.artifact_type} column {col} has invalid values: {sorted(bad)}")
    return frame
...
def validate_portfolio_ledger(df_or_path: pd.DataFrame | Path | str) -> pd.DataFrame:
    return _validate_dataframe(_normalize_portfolio_columns(_load_df(df_or_path)), PORTFOLIO_LEDGER_SCHEMA)
...
def reconcile_bundle_outputs(bundle_jsonl_path: Path | str, summary_df_or_path: pd.DataFrame | Path | str) -> dict[str, int]:
    bundle_path = Path(bundle_jsonl_path)
    lines = [json.loads(line) for line in bundle_path.read_text(encoding='utf-8').splitlines() if line.strip()]
...
def reconcile_portfolio_to_traces(portfolio_df_or_path: pd.DataFrame | Path | str, trace_dfs_or_paths: Sequence[pd.DataFrame | Path | str]) -> dict[str, float]:
    ...
    if not traces:
        return {'portfolio_total_net_pnl': float(portfolio['net_pnl'].sum()), 'trace_total_net_pnl': 0.0}
    total = None
    for trace in traces:
...
def reconcile_promoted_candidates(candidate_df_or_path: pd.DataFrame | Path | str, promotion_audit_df_or_path: pd.DataFrame | Path | str) -> dict[str, int]:
...
def validate_promotion_artifacts(root: Path | str) -> dict[str, Any]:
    ...
    if audit_path is None or promoted_path is None or bundle_summary_path is None or decisions_path is None or not bundle_jsonl.exists():
