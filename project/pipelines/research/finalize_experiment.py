...
def finalize_experiment(
                ...
                for p in run_reports.rglob("*.parquet"):
                    if "candidates" in p.name or "evaluated" in p.name:
                        try:
                            ...
                            if not df.empty and ("hypothesis_id" in df.columns or "candidate_id" in df.columns):
    ...
    if not results_df.empty:
        ...
        for _, row in results_df.iterrows():
            ...
            def get_status(r):
                ...
                if pd.isna(exp): return "empty_sample"
                if r.get("sample_size", 0) < 5 and r.get("n_obs", 0) < 5: return "insufficient_sample"
                return "evaluated"
        ...
        for _, row in merged_df.iterrows():
            ...
            if hid in eval_map:
                ...
                elif t_type == "sequence" and len(json.loads(row.get("trigger_payload", "{}")).get("events", [])) > 2:
                    r_data["eval_status"] = "unsupported_trigger_evaluator"
                else:
    ...
    eval_df = merged_df[~merged_df["eval_status"].isin(["not_executed_or_missing_data", "unsupported_trigger_evaluator"])].copy()
    ...
    summary = {
        ...
        "passed_hypotheses": int(merged_df.get("gate_phase2_final", pd.Series([False])).sum()) if "gate_phase2_final" in merged_df.columns else 0,
    }
