...
def compute_discovery_quality_kpis(candidates: pd.DataFrame, min_fdr: float = 0.05) -> dict:
    ...
    # Depth Coverage
    has_sequence = promoted["canonical_event_type"].astype(str).str.contains("->", regex=False).mean() if "canonical_event_type" in promoted.columns else np.nan
    num_logical_nodes = (promoted["condition"].astype(str).str.count(" and ").mean() + 1) if "condition" in promoted.columns and promoted["condition"].notna().any() else np.nan
    
    # Diversity
    families = promoted["canonical_family"].dropna().astype(str).value_counts(normalize=True) if "canonical_family" in promoted.columns else pd.Series(dtype=float)
