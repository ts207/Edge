...
def build_conditions(events: pd.DataFrame) -> List[ConditionSpec]:
    ...
    for col in config_cols:
        if col in events.columns:
            ...
            unique_vals = sorted(value_counts.index.tolist(), key=lambda v: (-int(value_counts.get(v, 0)), str(v)))
            if len(unique_vals) > MAX_DYNAMIC_CONDITIONS_PER_COLUMN:
                LOGGER.warning("Capping dynamic conditions for %s at %d of %d distinct values", col, MAX_DYNAMIC_CONDITIONS_PER_COLUMN, len(unique_vals))
...
def attach_event_market_features(
    ...
    for symbol, sub in out.sort_values(["symbol", "enter_ts"]).groupby("symbol", sort=False):
        ...
        if ctx.empty:
            ...
            continue
        merged_sub = pd.merge_asof(sub, ctx, on="enter_ts", direction="backward", suffixes=("", "_ctx"))
...
def prepare_baseline(events: pd.DataFrame, controls: pd.DataFrame) -> pd.DataFrame:
    ...
    merged["opportunity_proxy_excess"] = (merged["opportunity_proxy"] - merged["opportunity_proxy_ctrl"])
