...
def load_features(run_id: str, symbol: str, timeframe: str = "5m", higher_timeframes: list[str] | None = None, market: str = "perp", data_root: Path | None = None) -> pd.DataFrame:
...
def run_family_analyzer(*, family_name: str, event_types: Iterable[str], event_mask_fn: Callable, event_score_fn: Callable, default_min_spacing: int = 1, min_spacing_fn: Callable | None = None, load_features_fn: Callable = load_features) -> int:
    DATA_ROOT = get_data_root()
    parser = argparse.ArgumentParser(description=f"Family-specific analyzer for {family_name} events")
    ...
    for symbol in symbols:
        features = load_features_fn(run_id=str(args.run_id), symbol=symbol, timeframe=str(args.timeframe))
        if features.empty:
            continue
        ...
        if isinstance(result, pd.DataFrame):
            ...
            mask = result
            spacing = int(min_spacing_fn(event_type, spec) if min_spacing_fn is not None else default_min_spacing)
            part = rows_for_event(
                features,
                symbol=symbol,
        ...
        if not part.empty:
            ...
            feature_cols = [c for c in features.columns if c not in part.columns or c == "timestamp"]
            part = part.merge(
