...
def evaluate_family_promotion(
    ...
    *,
    ...
    integrity = getattr(analyzer_results.get('integrity'), 'summary', {}) if analyzer_results else {}
    edge = getattr(analyzer_results.get('edge'), 'summary', {}) if analyzer_results else {}
    stability = getattr(analyzer_results.get('stability'), 'summary', {}) if analyzer_results else {}
    ...
    if metrics['cluster_rate'] > max_cluster_rate:
        reasons.append(f"cluster rate too high: {metrics['cluster_rate']:.3f} > {max_cluster_rate:.3f}")
    if not metrics['pit_ok']:
        reasons.append('pit validation failed')
    if metrics['best_net_mean_bps'] < min_best_net_bps:
        reasons.append(f"edge too weak: {metrics['best_net_mean_bps']:.3f} < {min_best_net_bps:.3f}")
    if metrics['sign_consistency'] < min_sign_consistency:
        reasons.append(f"sign consistency too low: {metrics['sign_consistency']:.3f} < {min_sign_consistency:.3f}")
    if metrics['asset_mean_dispersion'] > max_asset_dispersion:
        reasons.append(f"asset dispersion too high: {metrics['asset_mean_dispersion']:.3f} > {max_asset_dispersion:.3f}")
    passed = not reasons
    return PromotionDecision(status='approved' if passed else 'prototype', passed=passed, reasons=reasons, metrics=metrics)
