"""
Bybit V5 Derivatives Open Interest history ingestion.
"""
...
async def _ingest_oi_month(
    ...
    async with semaphore:
        ...
        out_dir = out_root / symbol / "open_interest" / f"year={month_start.year}" / f"month={month_start.month:02d}"
        out_path = out_dir / f"oi_{symbol}_{month_start.year}-{month_start.month:02d}.parquet"

        if not force and out_path.exists():
            return {"status": "skipped", "partition": str(out_path), "count": 0}

        _LOG.info(f"Ingesting OI for {symbol} for {month_start.strftime('%Y-%m')}")
        ...
        while current_end > final_start:
            try:
                batch = await _fetch_oi(session, symbol, interval, final_start, current_end, limit=200)
                consecutive_errors = 0
                if not batch:
                    break

                all_records.extend(batch)
                oldest_in_batch = min(int(k["timestamp"]) for k in batch)
                ...
                if consecutive_errors >= max_retries:
                    _LOG.error(
                        f"Error fetching OI for {symbol} after {max_retries} attempts: {e}"
                    )
                    break
                wait = retry_backoff_sec * (2 ** (consecutive_errors - 1))
                _LOG.warning(
                    f"OI fetch error for {symbol} (attempt {consecutive_errors}/{max_retries}): "
...
async def async_main(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        for symbol in symbols:
            tasks = [
                _ingest_oi_month(
                    semaphore, session, symbol, args.interval, m, start_dt, end_dt, out_root, bool(args.force),
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Ingest Bybit Derivatives Open Interest history")
    parser.add_argument("--run_id", required=True)
    parser.add_argument("--symbols", required=True)
    parser.add_argument("--start", required=True)
    parser.add_argument("--end", required=True)
    parser.add_argument("--timeframe", default="1m")
    parser.add_argument("--data_type", default="oi")
    parser.add_argument("--interval", default="5min", help="5min, 15min, 30min, 1h, 4h, 1d")
    ...
    try:
        ...
        _LOG.exception(f"OI ingestion failed: {e}")
