...
from project.io.utils import choose_partition_dir, ensure_dir, list_parquet_files, read_parquet, write_parquet, run_scoped_lake_path
...
def main() -> int:
    ...
    params = {"symbols": symbols, "market": args.market, "timeframe": timeframe, "force": int(args.force)}
    ...
    try:
        ...
        for symbol in symbols:
            ...
            rows.append({"symbol": symbol, "listing_start": start_ts, "listing_end": end_ts, "market": args.market, "timeframe": timeframe})
            stats["symbols"][symbol] = {"rows": int(len(df)), "listed": True, "listing_start": start_ts.isoformat(), "listing_end": end_ts.isoformat(), "timeframe": timeframe}
            ...
            while month_cursor <= month_end:
                ...
                month_cursor = (month_cursor + pd.offsets.MonthBegin(1)).normalize().tz_localize("UTC") if month_cursor.tz is None else month_cursor + pd.offsets.MonthBegin(1)
            inputs.append({"path": str(run_scoped_lake_path(data_root, args.run_id, "cleaned", args.market, symbol, dataset)), "rows": int(len(df)), "start_ts": start_ts.isoformat(), "end_ts": end_ts.isoformat(), "timeframe": timeframe})

        snap_df = pd.DataFrame(rows)
        if not snap_df.empty:
            global_latest = pd.to_datetime(snap_df["listing_end"], utc=True, errors="coerce").max()
            active_threshold = global_latest - pd.Timedelta(days=7) if pd.notna(global_latest) else pd.Timestamp("1970-01-01", tz="UTC")
            snap_df["status"] = snap_df["listing_end"].apply(lambda ts: "active" if pd.to_datetime(ts, utc=True, errors="coerce") >= active_threshold else "inactive")
        else:
        ...
        base_out = Path(args.out_dir) if args.out_dir else data_root / "lake" / "runs" / args.run_id / "metadata" / "universe_snapshots" / timeframe
        ...
        outputs.append({"path": str(written), "rows": int(len(snap_df)), "start_ts": None, "end_ts": None, "storage": storage, "timeframe": timeframe})

        summary_payload = {"run_id": args.run_id, "market": args.market, "timeframe": timeframe, "dataset": dataset, "symbols_requested": symbols, "symbols_with_history": int(len(snap_df)), "snapshots": snap_df.assign(listing_start=snap_df["listing_start"].astype(str) if not snap_df.empty else pd.Series(dtype=str), listing_end=snap_df["listing_end"].astype(str) if not snap_df.empty else pd.Series(dtype=str)).to_dict(orient="records"), "monthly_membership": stats["monthly_membership"]}
