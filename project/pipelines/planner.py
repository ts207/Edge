...
DEPENDENCY_PATTERNS: List[Tuple[str, List[str]]] = [
    ...
    ("finalize_experiment", ["@PHASE2_DISCOVERY_STAGES"]),
...
def _resolve_dependencies(name: str, all_stage_names: List[str]) -> List[str]:
    ...
    tf = next((p for p in name.split("_") if p in {"1m", "5m", "15m", "30m", "1h", "4h", "1d"}), "5m")
    event = None
    if sep_idx >= 0:
            ...
            if "{script}" in pattern and name.startswith("analyze_") and sep_idx >= 0:
                ...
                if pattern.startswith("analyze_{script}__"):
                    ...
                    for dp in dep_patterns:
                        deps.append(dp.replace("{script}", script_part).replace("{event}", event or "").replace("{tf}", tf))
                ...
                if name.startswith(prefix) and name.endswith(suffix):
                    if not suffix.endswith("_spot") and name.endswith("_spot") and not pattern.endswith("_spot"):
                        continue
                    match = True
                    for dp in dep_patterns:
    ...
    for d in deps:
        if d == "@REGISTRY_STAGES":
            ...
            if event:
                resolved.extend([n for n in all_stage_names if n.startswith("analyze_") and f"__{event}_" in n])
            else:
                # Fallback extraction if event wasn't parsed via __
                event_part = name.replace("build_event_registry_", "").rsplit("_", 1)[0].lstrip("_")
                resolved.extend([n for n in all_stage_names if n.startswith("analyze_") and f"_{event_part}_" in n])
        elif d == "@FIRST_OHLCV_STAGE":
            ohlcv_stages = sorted([n for n in all_stage_names if n.startswith("ingest_binance_um_ohlcv_")])
            if ohlcv_stages:
    ...
    if name.startswith("analyze_"):
        ...
        family_stages = sorted([n for n in all_stage_names if n.startswith(family_prefix) and n.endswith(f"_{tf}")])
        try:
    ...
    if "canonicalize_event_episodes" in name:
        canonical_stages = sorted([n for n in all_stage_names if "canonicalize_event_episodes" in n])
        try:
...
def build_pipeline_plan(
    *,
    ...
    # Collect all stages
    raw_stages = []
    raw_stages.extend(build_ingest_stages(args=args, run_id=run_id, symbols=symbols, start=start, end=end, force_flag=force_flag, run_spot_pipeline=run_spot_pipeline, project_root=project_root))
    raw_stages.extend(build_core_stages(args=args, run_id=run_id, symbols=symbols, start=start, end=end, force_flag=force_flag, allow_missing_funding_flag=allow_missing_funding_flag, run_spot_pipeline=run_spot_pipeline, project_root=project_root))
    raw_stages.extend(build_research_stages(args=args, run_id=run_id, symbols=symbols, start=start, end=end, research_gate_profile=research_gate_profile, project_root=project_root, data_root=data_root, phase2_event_chain=phase2_event_chain))
    raw_stages.extend(build_evaluation_stages(args=args, run_id=run_id, symbols=symbols, start=start, end=end, force_flag=force_flag, project_root=project_root, data_root=data_root))
    ...
    for name, stage in plan.items():
        ...
        if name.startswith(stages_with_config_prefixes) or name == "compile_strategy_blueprints" or "phase2_conditional_hypotheses" in name:
        ...
        if "build_market_context" in name:
            if script_supports_flag(stage_script_path, "--max_symbol_workers"):
                _upsert_cli_flag(base_args, "--max_symbol_workers", str(int(args.market_context_workers)))
        
        if name == "select_profitable_strategies":
            if script_supports_flag(stage_script_path, "--objective_name"):
                _upsert_cli_flag(base_args, "--objective_name", str(getattr(args, "objective_name", "") or ""))

        if (name.startswith("phase2_conditional_hypotheses") or name == "compile_strategy_blueprints"):
            if args.fees_bps is not None: base_args.extend(["--fees_bps", str(args.fees_bps)])
