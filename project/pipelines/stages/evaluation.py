...
def build_evaluation_stages(
        ...
        stages.append(
            (
                ...
                [
                    "--run_id", run_id,
                    "--symbols", symbols,
                    "--blueprints_file", str(blueprints_path),
                    ...
                    "--enable_fractional_allocation", str(int(args.strategy_builder_enable_fractional_allocation)),
                ],
            )
        )
    ...
    if int(getattr(args, "run_profitable_selector", 1)):
        strategy_candidates_path = data_root / "reports" / "strategy_builder" / run_id / "strategy_candidates.parquet"
        stages.append(
            (
