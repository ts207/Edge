...
import numpy as np
import pandas as pd
from project.io.utils import choose_partition_dir, list_parquet_files, read_parquet, run_scoped_lake_path, ensure_dir
...
def eval_entropy(
    ...
    for col, name in cols.items():
        if col in df.columns:
            ...
            if len(counts) > 0:
                ...
                if modal_mass > max_modal_mass and col not in ["ms_vol_state", "ms_oi_state", "ms_spread_state"]:
    ...
    if len(code_counts) > 0:
        ...
        if unique_gt_1 < min_unique_codes:
            issues.append(f"Only {unique_gt_1} unique codes >= 1% (min {min_unique_codes}) for {symbol}")
    ...
    retained_masks = [info["representative"] for m, info in class_map.items() if not info["removed"]]
...
def main() -> int:
    ...
    for symbol in symbols:
        ...
        candidates = [
            run_scoped_lake_path(data_root, args.run_id, "context", "market_state", args.market, symbol, "5m"),
        ...
        report, issues = eval_entropy(
            df, 
            symbol,
            max_modal_mass=args.max_modal_mass
        )
