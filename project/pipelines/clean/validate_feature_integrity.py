...
def check_nans(df: pd.DataFrame, threshold: float = 0.05) -> List[str]:
    ...
    return [f"Column '{col}' has {pct:.2%} NaNs (threshold {threshold:.2%})" for col, pct in failing_cols.items()]
...
def check_outliers(df: pd.DataFrame, z_threshold: float = 10.0) -> List[str]:
    cols_to_check = [c for c in df.select_dtypes(include=[np.number]).columns if c not in ['timestamp', 'open', 'high', 'low', 'close', 'volume']]
    ...
    return [f"Column '{col}' has {pct:.2%} extreme outliers (> {z_threshold} sigma)" for col, pct in failing_cols.items()]
    ...
    *,
    ...
    if bars_dir:
        ...
        if not df_bars.empty:
            bars_issues = check_nans(df_bars, threshold=nan_threshold) + check_constant_values(df_bars)
            if bars_issues:
    ...
    if features_dir:
        ...
        if not df_feats.empty:
            ...
            for flag in drift_flags:
                feat_issues.append(f"Drift detected in '{flag['feature']}': KS p-value = {flag['p_value']:.4f}")
...
def main() -> int:
    ...
    manifest = start_manifest(f"validate_feature_integrity_{args.timeframe}", args.run_id, vars(args), [], [])
