...
def test_feature_prefix_invariance(module_path, func_name, gen_name):
    ...
    for cutoff in [100, 250, 400]:
        ...
        pd.testing.assert_series_equal(
            ...
            check_dtype=False, 
            atol=1e-10,
            obj=f"{func_name} at cutoff {cutoff}"
        )

def test_vol_shock_relaxation_causality():
    """VSR has a different signature (detect_vol_shock_relaxation_events)."""
    from project.features.vol_shock_relaxation import detect_vol_shock_relaxation_events, VolShockRelaxationConfig
...
def test_funding_persistence_causality():
    ...
    """
    from project.features.funding_persistence import build_funding_persistence_state, FundingPersistenceConfig
...
def test_rolling_center_not_used_in_feature_modules():
    ...
    features_dir = Path(__file__).resolve().parents[2] / "project" / "features"
