...
def test_filter_optimization_determinism():
    ...
    labels_train = pd.Series(df_train["feat2"] * 2.0 - df_train["feat3"] * 1.5 + np.random.normal(size=n_train) * 0.1)
    ...
    labels_val = pd.Series(df_val["feat2"] * 2.0 - df_val["feat3"] * 1.5 + np.random.normal(size=n_val) * 0.1)
