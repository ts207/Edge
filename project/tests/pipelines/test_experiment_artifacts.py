...
import json
import hashlib
from project.pipelines.research.experiment_engine import build_experiment_plan, export_experiment_artifacts
...
def test_env(tmp_path):
    ...
    (reg_dir / "events.yaml").write_text(yaml.dump({
        "events": {
            "VOL_SPIKE": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": True, "requires_features": []},
            "LIQ_GAP": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": True, "requires_features": []}
        }
    }))
    (reg_dir / "templates.yaml").write_text(yaml.dump({
        "templates": {
            "continuation": {"enabled": True, "supports_trigger_types": ["EVENT", "SEQUENCE"]}
        }
    }))
    (reg_dir / "states.yaml").write_text(yaml.dump({"states": {}}))
    (reg_dir / "features.yaml").write_text(yaml.dump({"features": {}}))
    (reg_dir / "contexts.yaml").write_text(yaml.dump({"context_dimensions": {"session": {"allowed_values": ["open", "close"]}}}))
    (reg_dir / "search_limits.yaml").write_text(yaml.dump({"limits": {"max_events_per_run": 10, "max_templates_per_run": 10, "max_horizons_per_run": 10, "max_directions_per_run": 10, "max_sequence_length": 5}}))
    (reg_dir / "detectors.yaml").write_text(yaml.dump({"detector_ownership": {"VOL_SPIKE": "VolSpikeDetector"}}))
...
def test_artifacts_contract(test_env):
    ...
    assert len(df) == 1
    expected_cols = {"hypothesis_id", "trigger_type", "trigger_payload", "template_id", "horizon", "direction", "entry_lag", "context_slice"}
