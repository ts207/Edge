...
from project.pipelines.research.template_regression import (  # noqa: E402
    build_run_summary,
    compare_summaries,
...
def test_compare_summaries_reports_differences():
    baseline = {
        "events": {"VOL_SHOCK": {"rows": 2, "templates": ["a"], "actions": ["x"], "direction_rules": ["d"]}}
    }
    current = {
        "events": {"VOL_SHOCK": {"rows": 3, "templates": ["a"], "actions": ["y"], "direction_rules": ["d"]}}
    }
...
def test_build_run_summary_reads_event_parquet(tmp_path):
    ...
    pd.DataFrame([{"template_verb": "continuation", "action": "enter_long_market", "direction_rule": "directional"}]).to_parquet(
