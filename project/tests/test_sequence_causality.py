...
def test_sequence_causality_and_earliest_match():
    ...
    assert row["signal_ts"] == pd.to_datetime("2024-01-01 10:10:00", utc=True), "signal_ts must map to last event in chain"
    
    # 3) Sequence enter_ts equals A's signal_ts (lineage)
    assert row["enter_ts"] == pd.to_datetime("2024-01-01 10:05:00", utc=True), "enter_ts must map to first event in chain"
