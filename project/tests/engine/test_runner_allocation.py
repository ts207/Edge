...
def test_run_engine_applies_allocator_before_final_pnl(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_run_engine_drops_legacy_alias_columns(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    def fake_get_strategy(_name: str):
        return _DummyStrategy()
