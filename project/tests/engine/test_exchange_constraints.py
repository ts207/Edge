...
def test_apply_constraints_clips_trade():
    ...
    result = apply_constraints(requested_qty=pd.Series([0.05]), price=pd.Series([50.0]), constraints=c)
...
def test_no_constraints_passthrough():
    ...
    assert apply_constraints(requested_qty=pd.Series([1.2345]), price=pd.Series([100.0]), constraints=c).iloc[0] == 1.2345

def test_negative_qty_sign_preserved():
    import pytest
    c = SymbolConstraints(tick_size=0.01, step_size=0.001, min_notional=10.0)
    result = apply_constraints(requested_qty=pd.Series([-1.2345]), price=pd.Series([100.0]), constraints=c)
    assert result.iloc[0] == pytest.approx(-1.234)

def test_negative_qty_min_notional_zero():
    c = SymbolConstraints(tick_size=0.01, step_size=0.001, min_notional=100.0)
    result = apply_constraints(requested_qty=pd.Series([-0.001]), price=pd.Series([50.0]), constraints=c)
