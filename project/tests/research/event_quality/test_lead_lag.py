...
def test_event_event_lead_lag_detects_causal_order():
    ...
    early_to_late = result[
        (result["source_event"] == "early") & (result["target_event"] == "late")
    ]
...
def test_event_event_lead_lag_reverse_is_low():
    ...
    late_to_early = result[
        (result["source_event"] == "late") & (result["target_event"] == "early")
        & (result["lag_bars"] <= 5)
    ]
...
def test_event_return_lead_lag_no_close_raises():
    df = pd.DataFrame({"timestamp": pd.date_range("2023-01-01", periods=10, freq="5min"), "event_x": True})
