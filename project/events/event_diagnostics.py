...
def generate_event_coverage_report(
    ...
    for evt_type in event_types:
        ...
        hits_median = (
            float(hits_per_symbol.median()) if len(hits_per_symbol) > 0 else 0.0
        )
...
def verify_index_alignment(
    ...
    mismatch_rate = (
        float(misaligned_count / len(event_ts)) * 100 if len(event_ts) > 0 else 0.0
    )
...
def registry_contract_check(
    events: pd.DataFrame, flags: pd.DataFrame, symbol: str
) -> None:
