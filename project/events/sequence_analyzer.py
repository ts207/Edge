...
def detect_sequences(
    ...
    for symbol, group in df.groupby("symbol"):
        ...
        event_indices = {
            etype: group.index[group["event_type"] == etype].tolist()
            for etype in set(events)
        }
...
def _find_next_step(
    ...
    candidates = [i for i in event_indices.get(next_event, []) if group.loc[i, "signal_ts"] > last_ts]
