...
def validate_timing_invariants(timing: EventTimingContract, entry_lag_bars: int = 1) -> None:
    ...
    if timing.active_start_time > timing.active_end_time:
        raise ValueError(f"Active interval never begins after it ends: {timing.active_start_time} > {timing.active_end_time}")
    
    if timing.first_tradable_bar_open_time < timing.signal_bar_open_time:
        raise ValueError(f"Tradable signal cannot precede signal bar: {timing.first_tradable_bar_open_time} < {timing.signal_bar_open_time}")
    
    # R1: earliest signalable/tradable bar is t+1 (detection + 1)
    if timing.first_tradable_bar_open_time <= timing.event_ts_snapped:
         raise ValueError(f"Tradable bar must be after detection bar open: {timing.first_tradable_bar_open_time} <= {timing.event_ts_snapped}")
