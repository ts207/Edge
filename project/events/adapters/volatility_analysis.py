...
from project.events.registries.volatility import VOLATILITY_DETECTORS, ensure_volatility_detectors_registered
from project.research.analyzers import run_analyzer_suite


def detect_volatility_family(df: pd.DataFrame, symbol: str, event_type: str, **params: Any) -> pd.DataFrame:
...
def analyze_volatility_family(df: pd.DataFrame, symbol: str, event_type: str, **params: Any) -> tuple[pd.DataFrame, dict[str, Any]]:
