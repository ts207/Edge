...
def prepare_flow_exhaustion_features(
    ...
    oi_drop_q = lagged_rolling_quantile(oi_drop, window=window, quantile=float(params.get("oi_drop_quantile", defaults["oi_drop_quantile"])), min_periods=min_periods)
    liq_q = lagged_rolling_quantile(liq_notional, window=window, quantile=float(params.get("liquidation_quantile", defaults["liquidation_quantile"])), min_periods=min_periods)
    spread_q = lagged_rolling_quantile(spread_bps, window=window, quantile=float(params.get("spread_quantile", defaults["spread_quantile"])), min_periods=min_periods)
    ret_q = lagged_rolling_quantile(ret_abs, window=window, quantile=float(params.get("return_quantile", defaults["return_quantile"])), min_periods=min_periods)
    
    rebound_ret = close.pct_change(periods=int(params.get("rebound_window", defaults["rebound_window"]))).fillna(0.0)
    reversal_impulse = close.pct_change(periods=int(params.get("reversal_window", defaults["reversal_window"]))).abs()
    reversal_q = lagged_rolling_quantile(reversal_impulse, window=window, quantile=float(params.get("reversal_quantile", defaults["reversal_quantile"])), min_periods=min_periods)
    ...
    liquidation_multiplier = float(params.get("liquidation_multiplier", defaults["liquidation_multiplier"]))
    ...
    forced_flow = (
        (
            ...
            & (
                liq_notional
                >= np.maximum(liq_q.fillna(0.0), liquidation_abs_min)
            ).fillna(False)
        )
        | (
            (
...
def prepare_post_deleveraging_features(
    ...
    window = int(params.get("threshold_window", defaults["threshold_window"]))
    min_periods = max(window // 10, 1)
    oi_drop_q80 = lagged_rolling_quantile(oi_drop, window=window, quantile=float(params.get("oi_drop_quantile", defaults["oi_drop_quantile"])), min_periods=min_periods)
    liq_q85 = lagged_rolling_quantile(liq_notional, window=window, quantile=float(params.get("liquidation_quantile", defaults["liquidation_quantile"])), min_periods=min_periods)
    spread_q70 = lagged_rolling_quantile(spread_bps, window=window, quantile=float(params.get("spread_quantile", defaults["spread_quantile"])), min_periods=min_periods)
    ret_q75 = lagged_rolling_quantile(ret_abs, window=window, quantile=float(params.get("return_quantile", defaults["return_quantile"])), min_periods=min_periods)
    wick_q70 = lagged_rolling_quantile(wick_ratio, window=window, quantile=float(params.get("wick_quantile", defaults["wick_quantile"])), min_periods=min_periods)
    ...
    rebound_ret_q70 = lagged_rolling_quantile(rebound_ret.abs(), window=window, quantile=float(params.get("rebound_quantile", defaults["rebound_quantile"])), min_periods=min_periods)
    ...
    reversal_q65 = lagged_rolling_quantile(reversal_impulse, window=window, quantile=float(params.get("reversal_quantile", defaults["reversal_quantile"])), min_periods=min_periods)
...
def compute_post_deleveraging_mask(
    ...
    post_cluster_lookback = int(params.get("post_cluster_lookback", defaults["post_cluster_lookback"]))
