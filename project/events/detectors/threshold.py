...
class ThresholdDetector(BaseEventDetector):
    ...
    def compute_threshold(self, df: pd.DataFrame, *, features: Mapping[str, pd.Series], **params: Any) -> pd.Series:
        ...
        if q is not None:
            ...
            return dynamic_quantile_floor(signal.abs(), window=window, quantile=float(q), floor=float(floor))
