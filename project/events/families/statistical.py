...
class StatisticalBase(ThresholdDetector):
    ...
    def compute_metadata(self, idx: int, features: dict[str, pd.Series], **params: Any) -> dict[str, Any]:
        ...
        return {
            ...
            **{k: float(v.iloc[idx]) if hasattr(v, 'iloc') else v for k, v in features.items() if k not in ['mask', 'intensity', 'close']}
        }
...
class OvershootDetector(StatisticalBase):
    ...
    def compute_raw_mask(self, df: pd.DataFrame, *, features: dict[str, pd.Series], **params: Any) -> pd.Series:
        ...
        return ((rv_z.shift(1) >= rv_q95).fillna(False) & (px_abs >= px_q95).fillna(False)).fillna(False)
...
def detect_statistical_family(df: pd.DataFrame, symbol: str, event_type: str, **params: Any) -> pd.DataFrame:
...
def analyze_statistical_family(df: pd.DataFrame, symbol: str, event_type: str, **params: Any) -> tuple[pd.DataFrame, dict[str, Any]]:
