...
def join_events_to_features(
    ...
    extra_evt_cols = [col for col in (
        "vol_regime", "liquidity_state", "market_liquidity_state", "depth_state",
        "event_direction", "direction", "signal_direction", "flow_direction",
        "breakout_direction", "shock_direction", "move_direction", "leader_direction",
        "return_1", "return_sign", "sign", "polarity", "funding_z", "basis_z",
        "side", "trade_side", "direction_label", "split_label"
...
def empty_expectancy_stats() -> Dict[str, Any]:
    return {
        "mean_return": 0.0, "p_value": 1.0, "n_events": 0.0, "n_effective": 0.0,
        "stability_pass": False, "std_return": 0.0, "t_stat": 0.0, "time_weight_sum": 0.0,
        "mean_weight_age_days": 0.0, "mean_tau_days": 0.0, "learning_rate_mean": 0.0,
        "mean_tau_up_days": 0.0, "mean_tau_down_days": 0.0, "tau_directional_ratio": 0.0,
        "directional_up_share": 0.0, "mean_train_return": 0.0, "mean_validation_return": 0.0,
        "mean_test_return": 0.0, "train_samples": 0, "validation_samples": 0, "test_samples": 0,
        "t_train": 0.0, "t_validation": 0.0, "t_test": 0.0,
    }
...
def _extract_event_returns(
    ...
    for row in merged.to_dict("records"):
        ...
        if pd.isna(feature_pos): continue
        pos = int(feature_pos)
        entry_pos = pos + int(entry_lag_bars)
        future_pos = entry_pos + horizon_bars + shift_labels_k
        if pos < 0 or pos >= len(feat_close) or entry_pos < 0 or entry_pos >= len(feat_close) or future_pos >= len(feat_close):
            continue
        ...
        event_direction = _event_direction_from_joined_row(row, canonical_family=canonical_family, fallback_direction=1)
...
def build_event_return_frame(
    ...
    *,
    ...
    horizon_bars = int(horizon_bars_override) if horizon_bars_override is not None else horizon_to_bars(horizon)
    ...
    for row in merged.to_dict("records"):
        ...
        if pos < 0 or entry_pos < 0 or entry_pos >= len(feat_close) or future_pos >= len(feat_close):
            continue
...
def _calculate_weights(
    ...
    if bool(regime_conditioned_decay) or bool(directional_asymmetry_decay):
        ...
        alpha = max(0.0, min(1.0, float(regime_tau_smoothing_alpha if bool(regime_conditioned_decay) else directional_tau_smoothing_alpha)))
        for vol_regime, liq_state, evt_dir in zip(event_vol_list, event_liq_list, event_dir_list):
            ...
            ) if bool(regime_conditioned_decay) else tau_seconds_default / 86400.0
            ...
            smoothed = raw_tau_days if prev_tau_days is None else ((1.0 - alpha) * prev_tau_days) + (alpha * raw_tau_days)
        ...
        return pd.Series(np.maximum(np.exp(-age_seconds / np.maximum(tau_seconds_arr, 1e-9)), float(time_decay_floor_weight)))
    else:
        return _time_decay_weights(ts_series, ref_ts, tau_seconds_default, float(time_decay_floor_weight))
...
def _calculate_tau_diagnostics(
    ...
    for vol_regime, liq_state, evt_dir in zip(event_vol_list, event_liq_list, event_dir_list):
        ...
        smoothed = tau_eff if prev_tau_days is None else ((1.0 - alpha) * prev_tau_days) + (alpha * tau_eff)
        tau_days_list.append(smoothed)
    ...
    up_share = float(np.mean([1.0 if int(x) >= 0 else 0.0 for x in event_dir_list])) if event_dir_list else 0.0
    return {
...
def calculate_expectancy_stats(
    ...
    extracted = _extract_event_returns(
        merged, features_df["close"].astype(float).values, 
        entry_lag_bars, horizon_bars, shift_labels_k,
        rule, side_policy, label_target, canonical_family
    )
    
    event_returns = extracted["returns"]
    if len(event_returns) < min_samples:
        return {**empty_expectancy_stats(), "n_events": float(len(event_returns)), "n_effective": float(len(event_returns))}
    ...
    if bool(time_decay_enabled):
        weights = _calculate_weights(
            ts_series, extracted["vol_regimes"], extracted["liq_states"], extracted["directions"],
            canonical_family, time_decay_tau_seconds, time_decay_floor_weight,
            regime_conditioned_decay, directional_asymmetry_decay,
            regime_tau_smoothing_alpha, directional_tau_smoothing_alpha,
            regime_tau_min_days, regime_tau_max_days,
            directional_tau_default_up_mult, directional_tau_default_down_mult,
            directional_tau_min_ratio, directional_tau_max_ratio
        )
    else:
        weights = pd.Series(1.0, index=returns_series.index)

    tau_diagnostics = _calculate_tau_diagnostics(
    ...
    ) if bool(time_decay_enabled) else {
        "mean_tau_days": 0.0,
        "learning_rate_mean": 0.0,
        "mean_tau_up_days": 0.0,
        "mean_tau_down_days": 0.0,
    ...
    mean_ret = float((returns_series * weights).sum() / w_sum) if w_sum > 0 else float(returns_series.mean())
    std_ret = float(np.sqrt(max(((weights * (returns_series - mean_ret) ** 2).sum()) / w_sum, 0.0))) if w_sum > 0 else float(returns_series.std())
    t_stat = float(mean_ret / (std_ret / np.sqrt(max(n_eff, 1.0)))) if std_ret > 0 and n_eff > 1 else 0.0
    ...
    return {
        "mean_return": mean_ret, "p_value": p_value, "n_events": float(len(event_returns)),
        "n_effective": float(n_eff), "std_return": std_ret, "t_stat": t_stat,
        "max_drawdown": dd_result["max_drawdown"], "gate_max_drawdown": dd_result["gate_max_drawdown"],
        **tau_diagnostics,
        "mean_train_return": float(np.mean([r for r, s in zip(event_returns, event_split_list) if s == "train"])) if "train" in event_split_list else 0.0,
        "mean_validation_return": float(np.mean([r for r, s in zip(event_returns, event_split_list) if s == "validation"])) if "validation" in event_split_list else 0.0,
        "mean_test_return": float(np.mean([r for r, s in zip(event_returns, event_split_list) if s == "test"])) if "test" in event_split_list else 0.0,
    }
...
def calculate_expectancy(
    ...
    stats_dict = calculate_expectancy_stats(sym_events, features_df, rule, horizon, shift_labels_k=shift_labels_k, entry_lag_bars=entry_lag_bars, min_samples=min_samples)
    return (float(stats_dict["mean_return"]), float(stats_dict["p_value"]), float(stats_dict["n_events"]), True)

def max_drawdown_gate(returns: List[float], *, max_dd_ratio: float = 3.0) -> dict:
    arr = np.asarray(returns, dtype="float64")
    arr = arr[np.isfinite(arr)]
    if arr.size < 2: return {"max_drawdown": 0.0, "dd_to_expectancy_ratio": 0.0, "gate_max_drawdown": True}
    ...
    return {"max_drawdown": max_dd, "dd_to_expectancy_ratio": ratio, "gate_max_drawdown": bool(ratio <= max_dd_ratio)}
