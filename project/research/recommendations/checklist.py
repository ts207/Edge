...
def gate_result(name: str, passed: bool, observed: Any, threshold: Any, note: str = "") -> Dict[str, Any]:
    return {
        "name": name,
        "passed": bool(passed),
        "observed": observed,
...
def build_checklist_payload(
    ...
    if not ok: reasons.append(f"edge candidates below threshold ({rows} < {min_rows})")
    ...
    if not ok: reasons.append(f"promoted edge candidates below threshold ({promoted} < {min_promoted})")
    ...
    if not ok: reasons.append(f"bridge-tradable candidates below threshold ({bridge} < {min_bridge})")
    ...
    if not ok: reasons.append("expectancy_exists is false")

    # 5. Robust Survivors
    survivors = robustness_payload.get("survivors", [])
    if not isinstance(survivors, list): survivors = robustness_payload.get("robust_survivors", [])
    count = len(survivors)
    min_surv = int(config.get("min_robust_survivors", 1))
    ok = count >= min_surv
    gates.append(gate_result("robust_survivor_count", ok, count, min_surv))
    if not ok: reasons.append(f"robust survivors below threshold ({count} < {min_surv})")
