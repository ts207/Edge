...
def build_release_signoff(
    *,
    ...
    observed_oos_sign = kpi_payload.get("metrics", {}).get("oos_sign_consistency", {}).get("value", 0.0)
    observed_drawdown = kpi_payload.get("metrics", {}).get("max_drawdown_pct", {}).get("value", 1.0)

    gates = []
    gates.append(gate_result("kpi_trade_count", observed_trade_count >= min_trade_count, observed_trade_count, min_trade_count))
    gates.append(gate_result("kpi_oos_sign_consistency", observed_oos_sign >= min_oos_sign_consistency, observed_oos_sign, min_oos_sign_consistency))
    gates.append(gate_result("kpi_max_drawdown_pct", abs(observed_drawdown) <= max_drawdown_pct, observed_drawdown, max_drawdown_pct))
