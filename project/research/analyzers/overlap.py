...
from project.research.analyzers.base import AnalyzerResult, BaseEventAnalyzer, ensure_timestamp, resolve_event_time_column


class OverlapAnalyzer(BaseEventAnalyzer):
    name = "overlap"

    def analyze(self, events: pd.DataFrame, *, market: pd.DataFrame | None = None, reference_events: pd.DataFrame | None = None, **kwargs: Any) -> AnalyzerResult:
        frame = self.validate_events(events)
        ref = self.validate_events(reference_events) if reference_events is not None else pd.DataFrame()
        if frame.empty or ref.empty:
            return AnalyzerResult(name=self.name, summary={"n_events": int(len(frame)), "reference_events": int(len(ref))}, tables={})
        ...
        table = pd.DataFrame({"timestamp": sorted(intersection)}) if intersection else pd.DataFrame(columns=["timestamp"])
