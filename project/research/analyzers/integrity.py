...
class IntegrityAnalyzer(BaseEventAnalyzer):
    ...
    def analyze(self, events: pd.DataFrame, *, market: pd.DataFrame | None = None, **kwargs: Any) -> AnalyzerResult:
        ...
        cluster_rate = float(((deltas_sec <= min_spacing_seconds) & deltas_sec.notna()).mean()) if len(frame) > 1 else 0.0

        by_month = {}
        if not frame.empty:
            month_key = frame[time_col].dt.strftime("%Y-%m")
            by_month = {str(k): int(v) for k, v in frame.groupby(month_key).size().to_dict().items()}

        missing_feature_rejections = int(frame.get("missing_feature_rejection", pd.Series(dtype=bool)).fillna(False).sum()) if "missing_feature_rejection" in frame.columns else 0
        warmup_discard_count = int(frame.get("warmup_discard", pd.Series(dtype=bool)).fillna(False).sum()) if "warmup_discard" in frame.columns else 0
        pit_ok = bool(frame.get("pit_ok", pd.Series([True] * len(frame))).fillna(True).all()) if len(frame) else True

        summary = {
            "n_events": int(len(frame)),
            "cluster_rate": cluster_rate,
            "avg_inter_event_seconds": float(np.nanmean(deltas_sec.to_numpy(dtype=float))) if len(frame) > 1 else None,
