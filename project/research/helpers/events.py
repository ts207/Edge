...
def rows_for_event(
    ...
    *,
    ...
    for n, idx in enumerate(idxs):
        ...
        severity = "moderate"
        if intensity >= 4.0: severity = "extreme"
        elif intensity <= 1.5: severity = "minor"
        ...
        metadata = {
            ...
            "funding_rate_bps": float(np.nan_to_num(safe_series(df, "funding_rate_scaled").iloc[idx], nan=0.0)),
        }
...
def past_quantile(series: pd.Series, q: float, window: int = 576, min_periods: int = 96) -> pd.Series:
    ...
    return rolling_quantile_threshold(series, window=int(window), quantile=float(q), min_periods=int(min_periods), shift=1)
