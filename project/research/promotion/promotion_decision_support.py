    ...
    *,
    ...
    if is_continuation_template_family and bridge_tradable:
        ...
        if not continuation_quality_pass:
            ...
            if not oos_pass:
                reasons.add_reject("continuation_quality_oos_validation", category="continuation_quality")
            if not microstructure_pass:
                reasons.add_reject("continuation_quality_microstructure", category="continuation_quality")
            if not dsr_pass:
                reasons.add_reject("continuation_quality_dsr", category="continuation_quality")
            reasons.add_promo_fail("gate_promo_continuation_quality", category="continuation_quality")
    return {
...
def _evaluate_control_audit_and_dsr(
    *,
    ...
    control_details = control_rate_details_for_event(row=row, event_type=event_type, summary=negative_control_summary)
...
def _evaluate_deploy_oos_and_low_capital(
    *,
    ...
    if is_deploy:
        ...
        if not cluster_pass:
            reasons.add_reject("multiplicity_cluster_q", category="deploy_confirmatory", deploy_only=True)
            multiplicity_pass = False
        if not by_pass:
            reasons.add_reject("multiplicity_by_diagnostic", category="deploy_confirmatory", deploy_only=True)
            multiplicity_pass = False
        if not shrinkage_loso_stable:
            reasons.add_reject("shrinkage_loso_unstable", category="deploy_confirmatory", deploy_only=True)
            multiplicity_pass = False
        if shrinkage_borrowing_dominant:
            reasons.add_reject("shrinkage_borrowing_dominant", category="deploy_confirmatory", deploy_only=True)
            multiplicity_pass = False
        if not robustness_panel_complete:
            reasons.add_reject("robustness_panel_incomplete", category="deploy_confirmatory", deploy_only=True)
            robustness_pass = False
        elif (not np.isfinite(structural_robustness_score)) or structural_robustness_score < 0.6:
            reasons.add_reject("robustness_structural_low", category="deploy_confirmatory", deploy_only=True)
            robustness_pass = False
        if (not np.isfinite(repeated_fold_consistency)) or repeated_fold_consistency < 0.5:
            reasons.add_reject("temporal_consistency_low", category="deploy_confirmatory", deploy_only=True)
            robustness_pass = False
        ...
        if num_regimes < min_regimes_req and not as_bool(row.get("is_regime_specific", False)):
            reasons.add_reject(f"regime_thin_support (regimes={num_regimes} < {min_regimes_req})", category="deploy_confirmatory", deploy_only=True)
            regime_pass = False
        if not regime_stability_pass:
            reasons.add_reject("regime_instability", category="deploy_confirmatory", deploy_only=True)
            regime_pass = False
        if not structural_break_pass:
            reasons.add_reject("structural_break_detected", category="deploy_confirmatory", deploy_only=True)
            regime_pass = False
        if not as_bool(row.get("bridge_certified", False)):
            reasons.add_reject("bridge_uncertified", category="deploy_confirmatory", deploy_only=True)
            robustness_pass = False
    ...
    if _has_explicit_oos_samples(row):
        ...
        if not oos_pass:
            if validation_samples < min_val_events or test_samples < min_test_events:
                reasons.add_reject(f"oos_insufficient_samples (val={validation_samples}, test={test_samples})", category="oos_validation")
