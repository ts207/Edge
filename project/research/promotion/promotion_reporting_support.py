...
def resolve_promotion_tier(
    ...
    *,
    ...
    if track == "standard" and retail_gate_ok and redundancy_ok and stat_ok and samples_ok and bridge_viability_ok:
        return "deployable"
    return "shadow"
...
def build_promotion_capital_footprint(
    *,
    ...
    target_account_size_usd = _quiet_float(getattr(contract, "target_account_size_usd", None), np.nan)
    capital_budget_usd = _quiet_float(getattr(contract, "capital_budget_usd", None), np.nan)
    per_position_cap_usd = _quiet_float(getattr(contract, "effective_per_position_notional_cap_usd", None), np.nan)
    ...
    for row in promoted_df.to_dict(orient="records"):
        ...
        if np.isfinite(capacity_proxy) and capacity_proxy > 0.0:
            usage_signal, usage_signal_source = float(np.clip(capacity_proxy, 0.0, 1.0)), "capacity_proxy"
        elif np.isfinite(turnover_proxy_mean) and np.isfinite(turnover_cap) and turnover_cap > 0.0:
            usage_signal, usage_signal_source = float(np.clip(turnover_proxy_mean / turnover_cap, 0.0, 1.0)), "turnover_ratio"
        elif np.isfinite(turnover_proxy_mean):
            usage_signal, usage_signal_source = float(np.clip(turnover_proxy_mean / 10.0, 0.0, 1.0)), "turnover_proxy_fallback"

        est_notional = capital_budget_usd * usage_signal if np.isfinite(capital_budget_usd) and np.isfinite(usage_signal) else np.nan
        slot_pressure = est_notional / per_position_cap_usd if np.isfinite(est_notional) and np.isfinite(per_position_cap_usd) and per_position_cap_usd > 0.0 else np.nan
        leverage_usage = est_notional / target_account_size_usd if np.isfinite(est_notional) and np.isfinite(target_account_size_usd) and target_account_size_usd > 0.0 else np.nan
        ...
        rows.append(
            {
                ...
                "target_account_size_usd": None if not np.isfinite(target_account_size_usd) else float(target_account_size_usd),
                "capital_budget_usd": None if not np.isfinite(capital_budget_usd) else float(capital_budget_usd),
                "per_position_notional_cap_usd": None if not np.isfinite(per_position_cap_usd) else float(per_position_cap_usd),
                "max_concurrent_positions": int(max_concurrent_positions),
                "turnover_proxy_mean": None if not np.isfinite(turnover_proxy_mean) else float(turnover_proxy_mean),
                "capacity_proxy": None if not np.isfinite(capacity_proxy) else float(capacity_proxy),
                "estimated_position_notional_usd": None if not np.isfinite(est_notional) else float(est_notional),
                "slot_pressure_fraction": None if not np.isfinite(slot_pressure) else float(slot_pressure),
                "leverage_usage_fraction": None if not np.isfinite(leverage_usage) else float(leverage_usage),
                "gate_capital_slot_within_limit": bool(slot_pressure <= 1.0) if np.isfinite(slot_pressure) else False,
                "gate_capital_leverage_within_budget": bool(usage_signal <= 1.0) if np.isfinite(usage_signal) else False,
            }
        )

    out_df = pd.DataFrame(rows, columns=cols)
    est_notional_s = pd.to_numeric(out_df.get("estimated_position_notional_usd", pd.Series(dtype=float)), errors="coerce")
    summary = {
        "promoted_count": int(len(out_df)),
        "estimated_notional_count": int(est_notional_s.notna().sum()),
        "slot_pressure_over_limit_count": int((pd.to_numeric(out_df["slot_pressure_fraction"], errors="coerce") > 1.0).sum()),
        "leverage_over_budget_count": int((pd.to_numeric(out_df["leverage_usage_fraction"], errors="coerce") > 1.0).sum()),
        "mean_estimated_position_notional_usd": 0.0 if est_notional_s.dropna().empty else float(est_notional_s.dropna().mean()),
    }
    return out_df, summary


def behavior_key(row: Dict[str, Any]) -> Tuple[str, ...]:
    fields = ("event_type", "event", "symbol", "condition", "action", "direction_rule", "horizon", "template_verb", "state_id", "family_id")
    return tuple(str(row.get(f, "")).strip().lower() for f in fields)


def behavior_token_set(row: Dict[str, Any]) -> set[str]:
    tokens: set[str] = set()
    fields = ("event_type", "event", "symbol", "condition", "action", "direction_rule", "horizon", "template_verb", "state_id", "family_id", "condition_label")
    for field in fields:
...
def apply_portfolio_overlap_gate(
    *,
    ...
    if promoted_df.empty:
        return promoted_df.copy(), pd.DataFrame(columns=["candidate_id", "event_type", "overlap_with_candidate_id", "overlap_score", "overlap_reason"])
    sort_cols, ascending = [], []
    for col, asc in (("selection_score", False), ("promotion_score", False), ("robustness_score", False), ("n_events", False), ("candidate_id", True)):
        if col in promoted_df.columns:
            sort_cols.append(col)
            ascending.append(asc)
    ranked = promoted_df.sort_values(sort_cols, ascending=ascending).reset_index(drop=True) if sort_cols else promoted_df.reset_index(drop=True)
    ...
    for row in ranked.to_dict(orient="records"):
        ...
        for selected in selected_rows:
            ...
            if np.isfinite(score) and (not np.isfinite(worst_overlap) or score > worst_overlap):
                worst_overlap, worst_with = float(score), str(selected.get("candidate_id", "")).strip()
    ...
    return pd.DataFrame(selected_rows, columns=ranked.columns), pd.DataFrame(dropped_rows, columns=["candidate_id", "event_type", "overlap_with_candidate_id", "overlap_score", "overlap_reason"])
    ...
    *,
    ...
    for i in range(len(rows)):
        for j in range(i + 1, len(rows)):
            ...
            lid, rid = str(left.get("candidate_id", "")).strip(), str(right.get("candidate_id", "")).strip()
            pair_total += 1
            corr = delay_profile_correlation(left, right)
            if np.isfinite(corr):
                corr_eval += 1
                if corr >= float(max_profile_correlation):
                    corr_violations.append({"left_candidate_id": lid, "right_candidate_id": rid, "profile_correlation": float(corr)})
            overlap = behavior_overlap_score(left, right)
            if np.isfinite(overlap):
                overlap_eval += 1
                if overlap >= float(max_overlap_ratio):
                    overlap_violations.append({"left_candidate_id": lid, "right_candidate_id": rid, "overlap_ratio": float(overlap)})
    return {
...
def assign_and_validate_promotion_tiers(
    *,
    ...
    if not promoted_df.empty:
        ...
        promoted_df["promotion_tier"] = [_get_tier(r) for r in promoted_df.to_dict(orient="records")]
    ...
    tier_counts = audit_df.get("promotion_tier", pd.Series(dtype="object")).astype(str).value_counts().to_dict()
...
def stabilize_promoted_output_schema(
    *,
    ...
    if out.empty:
        ...
        dtype_by_col = {
            c: source[c].dtype if c in source.columns else (out[c].dtype if c in out.columns else "object")
            for c in merged_cols
        }
        return pd.DataFrame({c: pd.Series(dtype=dtype_by_col[c]) for c in merged_cols}, columns=merged_cols)
