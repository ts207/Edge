...
def build_cluster_summary(clustered_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = [c for c in ["selection_score", "promotion_score", "n_events", "candidate_id"] if c in clustered_df.columns]
    ...
    for cid, group in clustered_df.groupby("cluster_id"):
        ...
        summary_rows.append({
            ...
            "avg_selection_score": group["selection_score"].mean() if "selection_score" in group else np.nan,
            "member_ids": ",".join(group["candidate_id"].tolist()[:10]) + ("..." if len(group) > 10 else ""),
        })
