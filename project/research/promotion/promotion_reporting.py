...
def build_negative_control_diagnostics(
    *,
    ...
    output["audit"]["promoted_with_missing_control_rate"] = int((promoted_mask & rates.isna()).sum())

    event_col = "event_type" if "event_type" in audit_df.columns else ("event" if "event" in audit_df.columns else None)
    ...
    for event_type, sub in audit_df.groupby(event_col, sort=True):
        ...
        sources = (
            sub.get("control_rate_source", pd.Series(dtype="object")).astype(str).fillna("").value_counts().to_dict()
        )
        by_event_rows[str(event_type)] = {
            "candidate_count": int(len(sub)),
            "promoted_count": int((sub.get("promotion_decision", pd.Series(dtype="object")).astype(str) == "promoted").sum()),
            "control_rate_missing_count": int(sub_rates.isna().sum()),
            "control_rate_mean": None if sub_rates.dropna().empty else float(sub_rates.dropna().mean()),
            "control_rate_max": None if sub_rates.dropna().empty else float(sub_rates.dropna().max()),
...
def build_promotion_statistical_audit(
    *,
    ...
    cols = [
        ...
        "cost_survival_ratio", "control_pass_rate", "control_rate_source", "tob_coverage",
        "validation_samples", "net_expectancy_bps", "effective_cost_bps", "turnover_proxy_mean",
        "low_capital_viability_score", "low_capital_reject_reason_codes", "promotion_score",
        "gate_bridge_tradable", "gate_promo_statistical", "gate_promo_multiplicity_diagnostics",
        "gate_promo_stability", "gate_promo_cost_survival", "gate_promo_negative_control",
        "gate_promo_falsification", "gate_promo_hypothesis_audit", "gate_promo_tob_coverage",
        "gate_promo_oos_validation", "gate_promo_microstructure", "gate_promo_retail_viability",
        "gate_promo_low_capital_viability", "gate_promo_timeframe_consensus",
        "gate_promo_event_discipline", "gate_promo_continuation_quality", "regime_flip_flag",
        "cross_symbol_sign_consistency", "rolling_instability_score", "bundle_version",
        "policy_version", "evidence_bundle_json", "promotion_metrics_trace",
    ]
    if audit_df.empty:
    ...
    for row in audit_df.to_dict(orient="records"):
        ...
        if decision == "rejected" and not primary_fail:
            primary_fail = "gate_promo_redundancy" if "redundancy_gate" in reason else "gate_promo_unknown"
        ...
        min_events_threshold = _quiet_int(row.get("promotion_min_events_threshold", row.get("n_events", 0)), 0)
        trace = {
            "statistical": {
                "passed": bool(row.get("gate_promo_statistical") == "pass"),
                "observed": {"q_value": _quiet_float(row.get("q_value"), np.nan), "n_events": _quiet_int(row.get("n_events"), 0)},
                "thresholds": {"max_q_value": float(max_q_value), "min_events": int(min_events_threshold)},
            },
            "stability": {
                "passed": bool(row.get("gate_promo_stability") == "pass"),
                "observed": {"stability_score": _quiet_float(row.get("stability_score"), np.nan), "sign_consistency": _quiet_float(row.get("sign_consistency"), np.nan)},
                "thresholds": {"min_stability_score": float(min_stability_score), "min_sign_consistency": float(min_sign_consistency)},
            },
            "cost_survival": {
                "passed": bool(row.get("gate_promo_cost_survival") == "pass"),
                "observed": {"cost_survival_ratio": _quiet_float(row.get("cost_survival_ratio"), np.nan)},
                "thresholds": {"min_cost_survival_ratio": float(min_cost_survival_ratio)},
            },
            "negative_control": {
                "passed": bool(row.get("gate_promo_negative_control") == "pass"),
                "observed": {"control_pass_rate": _quiet_float(row.get("control_pass_rate"), np.nan), "control_rate_source": str(row.get("control_rate_source", ""))},
                "thresholds": {"max_negative_control_pass_rate": float(max_negative_control_pass_rate), "allow_missing_negative_controls": bool(allow_missing_negative_controls)},
            },
            "oos_validation": {
            ...
            "continuation_quality": {
                ...
                "observed": {"template_verb": str(row.get("template_verb", "")), "gate_bridge_tradable": str(row.get("gate_bridge_tradable", "fail"))},
                "thresholds": {"continuation_family_requires_stability_oos_microstructure_dsr": True},
            },
            "retail": {
                ...
                "thresholds": {
                    ...
                    "max_fee_plus_slippage_bps": None if max_fee_plus_slippage_bps is None else float(max_fee_plus_slippage_bps),
                    "max_daily_turnover_multiple": None if max_daily_turnover_multiple is None else float(max_daily_turnover_multiple),
            ...
            "low_capital": {
                ...
                "observed": {
                    "low_capital_viability_score": _quiet_float(row.get("low_capital_viability_score"), np.nan),
                    "low_capital_reject_reason_codes": [token.strip() for token in str(row.get("low_capital_reject_reason_codes", "")).split(",") if token.strip()],
                },
                "thresholds": {"require_low_capital_viability": bool(require_low_capital_viability)},
            },
            "hypothesis_audit": {
            ...
            "bundle_policy": {
                ...
                "observed": {
                    ...
                    "cross_symbol_sign_consistency": _quiet_float(row.get("cross_symbol_sign_consistency"), np.nan),
                    "rolling_instability_score": _quiet_float(row.get("rolling_instability_score"), np.nan),
                },
                "thresholds": {"bundle_version": str(row.get("bundle_version", "")), "policy_version": str(row.get("policy_version", ""))},
            },
        }
        ...
        records.append(
            {
                ...
                "promotion_tier": str(row.get("promotion_tier", resolve_promotion_tier(row))).strip(),
                ...
                "turnover_proxy_mean": _quiet_float(row.get("turnover_proxy_mean"), np.nan),
                "low_capital_viability_score": _quiet_float(row.get("low_capital_viability_score"), np.nan),
                "low_capital_reject_reason_codes": str(row.get("low_capital_reject_reason_codes", "")),
