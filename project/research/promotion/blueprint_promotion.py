...
def calculate_drawdown_metrics(frame: pd.DataFrame, split_label: str) -> Dict[str, float]:
    ...
    if sub.empty:
        return {"max_loss_cluster_len": 0.0, "cluster_loss_concentration": 0.0, "tail_conditional_drawdown_95": 0.0}
    ...
    for idx, value in enumerate(values):
        if value < 0.0 and start is None: start = idx
        elif value >= 0.0 and start is not None:
            loss_magnitudes.append(float(np.abs(values[start:idx].sum())))
            start = None
    if start is not None: loss_magnitudes.append(float(np.abs(values[start:].sum())))
...
def fragility_gate(pnl_series: pd.Series, min_pass_rate: float = 0.60) -> bool:
    if pnl_series.empty: return False
