...
def evaluate_timeframe_consensus(
    ...
    for tf in alternate_timeframes:
        ...
        if alt_exp_key in row:
            ...
            sign_match = (base_expectancy > 0 and alt_exp > 0) or (base_expectancy < 0 and alt_exp < 0)
            ...
            if sign_match and exp_retention >= min_consensus_ratio and sharpe_retention >= min_sharpe_retention:
                passes += 1
                results[tf] = {"pass": True, "exp_retention": exp_retention, "sharpe_retention": sharpe_retention}
            else:
                results[tf] = {
                    "pass": False, 
            ...
            _LOG.debug(f"Missing alternate timeframe data for {tf} on candidate {row.get('candidate_id')}")
            passes += 1
