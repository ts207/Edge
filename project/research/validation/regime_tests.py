...
def compute_regime_labels(df: pd.DataFrame, *, vol_col: str = 'vol_regime', out_col: str = 'regime') -> pd.DataFrame:
    out = df.copy()
...
def evaluate_by_regime(df: pd.DataFrame, *, value_col: str, regime_col: str = 'regime') -> Dict[str, Any]:
    if df.empty or value_col not in df.columns or regime_col not in df.columns:
        return {'by_regime': {}, 'regime_flip_flag': False, 'worst_regime_estimate': 0.0, 'num_regimes': 0}
...
def evaluate_cross_symbol_stability(df: pd.DataFrame, *, value_col: str, symbol_col: str = 'symbol') -> Dict[str, Any]:
    if df.empty or value_col not in df.columns or symbol_col not in df.columns:
        return {'cross_symbol_sign_consistency': 0.0, 'worst_symbol_estimate': 0.0, 'by_symbol': {}, 'n_symbols': 0}
    ...
    if base_sign == 0.0:
        ...
        consistency = float(np.mean([(np.sign(v) == base_sign) if abs(v) > 1e-12 else False for v in means.values]))
...
def rolling_stability_metrics(values: Iterable[float], *, window: int = 3) -> Dict[str, Any]:
    ...
    if len(series) < window:
        score = float(series.std(ddof=0) / max(abs(series.mean()), 1e-9)) if len(series) > 1 else 0.0
...
def build_stability_result_from_row(row: Dict[str, Any]) -> StabilityResult:
    effect = safe_float(row.get('effect_shrunk_state', row.get('expectancy', row.get('estimate', 0.0))), 0.0)
    ...
    if not np.isfinite(sign_consistency):
        t_stats = [safe_float(row.get(k), np.nan) for k in ('train_t_stat', 'val_t_stat', 'oos1_t_stat', 'test_t_stat')]
        ...
        if t_stats and base_sign != 0.0:
            sign_consistency = float(np.mean([(np.sign(t) == base_sign) for t in t_stats if abs(t) > 1e-12]))
        else:
            sign_consistency = 0.0
    stability_score = safe_float(row.get('stability_score'), np.nan)
    if not np.isfinite(stability_score):
        stability_score = float(sign_consistency * (abs(effect) / max(std_return, 1e-8))) if np.isfinite(std_return) else 0.0
    ...
    regime_info = {'by_regime': {k: {'estimate': float(v), 'n_obs': int(regime_counts.get(k, 0))} for k, v in regime_map.items()}}
    if regime_map:
    ...
    if not symbol_map and 'cross_symbol_sign_consistency' in row:
        ...
        overall = effect
        base_sign = np.sign(overall) if abs(overall) > 1e-12 else 0.0
        cross_symbol_consistency = float(np.mean([(np.sign(v) == base_sign) for v in symbol_map.values() if abs(v) > 1e-12])) if symbol_map and base_sign != 0.0 else 0.0
        worst_symbol_estimate = float(min(symbol_map.values(), default=0.0))
    path = [safe_float(row.get(k), np.nan) for k in ('mean_train_return', 'mean_validation_return', 'mean_test_return')]
    path = [v for v in path if np.isfinite(v)]
    rolling = rolling_stability_metrics(path, window=2)
    return StabilityResult(
        sign_consistency=float(sign_consistency),
        stability_score=float(stability_score),
        regime_stability_pass=as_bool(row.get('gate_regime_stability', row.get('gate_stability', False))),
