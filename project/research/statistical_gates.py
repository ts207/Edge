...
def gate_redundancy_correlation(
    ...
    for i, row in df.iterrows():
        ...
        if not redundant: accepted_profiles.append(profile_vec)
...
def delay_robustness_fields(
    ...
    *,
    ...
    if not delay_expectancies_adjusted:
        return {"delay_positive_ratio": 0.0, "delay_dispersion": 0.0, "delay_robustness_score": 0.0, "gate_delay_robustness": False}
    ...
    gate = bool(delay_positive_ratio >= min_delay_positive_ratio and score >= min_delay_robustness_score)
    return {"delay_positive_ratio": delay_positive_ratio, "delay_dispersion": delay_dispersion, "delay_robustness_score": score, "gate_delay_robustness": gate}

def effective_sample_size(values: np.ndarray, max_lag: int) -> Tuple[float, int]:
    arr = np.asarray(values, dtype=float)
    arr = arr[np.isfinite(arr)]
    n = len(arr)
    if n <= 1: return float(n), 0
    lag_cap = min(int(max_lag), n // 10)
    if lag_cap <= 0: return float(n), 0
    rho_sum = sum(pd.Series(arr).autocorr(lag=lag) for lag in range(1, lag_cap + 1) if np.isfinite(pd.Series(arr).autocorr(lag=lag)))
...
def multiplicity_penalty(*, multiplicity_k: float, num_tests_event_family: int, ess_effective: float) -> float:
    return float(float(multiplicity_k) * np.sqrt(np.log(max(1.0, float(num_tests_event_family))) / max(1.0, float(ess_effective))))
...
def apply_multiplicity_adjustments(
    ...
    *,
    ...
    out = candidates.copy()
    out["multiplicity_penalty"] = out.apply(lambda r: multiplicity_penalty(multiplicity_k=multiplicity_k, num_tests_event_family=int(r.get("num_tests_event_family",0)), ess_effective=float(r.get("ess_effective",0.0))), axis=1)
    out["expectancy_after_multiplicity"] = pd.to_numeric(out.get("expectancy_per_trade", 0.0)) - out["multiplicity_penalty"]
    ...
    for _, row in out.iterrows():
        try:
            ...
            raw_map = {}
        adj_map = {str(k): float(safe_float(v, 0.0) - safe_float(row.get("multiplicity_penalty"), 0.0)) for k, v in raw_map.items()}
        fields = delay_robustness_fields(list(adj_map.values()), min_delay_positive_ratio=min_delay_positive_ratio, min_delay_robustness_score=min_delay_robustness_score)
