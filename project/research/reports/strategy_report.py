...
def generate_strategy_summary(
    ...
    lines.append("=" * 40)
    lines.append(f"Thesis: {blueprint.get('event_type', 'N/A')} with {blueprint.get('direction', 'N/A')} orientation.")
    ...
    lines.append(f"  Walk-forward Sign Consistency: {walkforward_results.get('sign_consistency', 0.0):.1%}")
    ...
    kill_criteria = blueprint.get('execution', {}).get('kill_criteria', [
...
def write_promotion_rationale(
    ...
    payload = {
        ...
        "kill_switch_config": blueprint.get("execution", {}).get("retry_logic"), # Reusing field for policy
    }
