...
def certify_benchmark_review(
    *,
    ...
    current_slices = {s["benchmark_id"]: s for s in current_review.get("slices", []) if s.get("benchmark_id")}
    ...
    latest_prior_slices = {s["benchmark_id"]: s for s in latest_prior.get("slices", []) if s.get("benchmark_id")}

    status_order = ["coverage_limited", "foundation_only", "informative", "quality_boundary", "authoritative"]
    ...
    if latest_prior_slices:
        for bid, curr in current_slices.items():
            ...
            # Status regression
            curr_rank = status_order.index(curr.get("benchmark_status", "coverage_limited")) if curr.get("benchmark_status") in status_order else -1
            prev_rank = status_order.index(prev.get("benchmark_status", "coverage_limited")) if prev.get("benchmark_status") in status_order else -1
    ...
    if len(priors) > 1:
        for bid, curr in current_slices.items():
            ...
            for p in priors:
                pslices = {s["benchmark_id"]: s for s in p.get("slices", []) if s.get("benchmark_id")}
...
def render_certification_report_markdown(cert: Dict[str, Any]) -> str:
    ...
    if cert.get("issues"):
        ...
        for i in cert["issues"]:
            lines.append(f"| `{i.get('benchmark_id')}` | `{i.get('severity')}` | `{i.get('type')}` | {i.get('message')} |")
        lines.append("")
    ...
    if cert.get("historical_drift"):
        ...
        for d in cert["historical_drift"]:
            ...
            lines.append(f"| `{d.get('benchmark_id')}` | {counts_str} | {d.get('mean'):.2f} | {d.get('std'):.2f} |")
        lines.append("")
...
def get_benchmark_certification_for_family(
    *,
    ...
    CERT_DEFAULT = Path("/tmp/benchmark_research_family_v1_full_live_20260318/benchmark_certification.json")
    REVIEW_DEFAULT = Path("/tmp/benchmark_research_family_v1_full_live_20260318/benchmark_review.json")
    
    cp = certification_path or CERT_DEFAULT
    rp = review_path or REVIEW_DEFAULT
    
    if not rp.exists():
        return {"passed": False, "status": "missing", "message": f"Benchmark review not found at {rp}"}
    ...
    if not target_slice:
        return {"passed": False, "status": "no_coverage", "message": f"No benchmark slice defined for family {family}"}
        
    bid = target_slice.get("benchmark_id")
    
    if cert and not cert.get("passed", False):
        slice_issues = [i for i in cert.get("issues", []) if i.get("benchmark_id") == bid and i.get("severity") == "fail"]
        if slice_issues:
            return {
                "passed": False, 
                "status": "failed", 
