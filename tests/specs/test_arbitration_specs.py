...
def test_compatibility_yaml_valid():
    ...
    for rule in data["suppression_rules"]:
        assert "penalty_factor" in rule, f"Missing penalty_factor in rule for {rule.get('when_active')}"
        assert isinstance(rule["penalty_factor"], float), f"penalty_factor must be float in rule for {rule.get('when_active')}"
        assert 0.0 <= rule["penalty_factor"] <= 1.0, f"penalty_factor out of range in rule for {rule.get('when_active')}"
        assert "block" in rule, f"Missing block in rule for {rule.get('when_active')}"
        assert isinstance(rule["block"], bool), f"block must be bool in rule for {rule.get('when_active')}"
...
def test_precedence_yaml_valid():
    ...
    for entry in data["family_precedence"]:
        assert isinstance(entry["priority"], int), f"priority must be int for family {entry.get('family')}"
    for override in data.get("event_overrides", []):
        assert "override_priority" in override
        assert isinstance(override["override_priority"], int), f"override_priority must be int for {override.get('event_type')}"
...
def _load_known_event_types() -> set:
    ...
    for p in SPEC_DIR.glob("*.yaml"):
        if p.name.startswith("_") or p.name in (
            "compatibility.yaml", "precedence.yaml",
            "canonical_event_registry.yaml", "event_registry_unified.yaml",
            "DESIGN.yaml",
        ):
            continue
        try:
