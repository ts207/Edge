...
def test_reconcile_portfolio_to_traces():
    portfolio = pd.DataFrame({
        ...
        'gross_pnl': [0.0, 0.02], 'net_pnl': [0.0, 0.01], 'equity': [1.0, 1.01], 'equity_return': [0.0, 0.01],
        'gross_exposure': [0.0, 1.0], 'net_exposure': [0.0, 1.0], 'turnover': [0.0, 1.0],
    })
    trace = pd.DataFrame({
        'timestamp': pd.date_range('2024-01-01', periods=2, freq='5min', tz='UTC'),
        'strategy': ['s', 's'], 'symbol': ['BTC', 'BTC'], 'signal_position': [0, 1], 'target_position': [0, 1], 'executed_position': [0, 1],
        'gross_pnl': [0.0, 0.015], 'net_pnl': [0.0, 0.01], 'equity_return': [0.0, 0.01],
    })
