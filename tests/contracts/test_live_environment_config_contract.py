...
def test_live_environment_configs_use_distinct_order_sources_and_snapshot_paths() -> None:
    ...
    assert float(paper["account_sync_interval_seconds"]) > float(production["account_sync_interval_seconds"])
    assert int(paper["account_sync_failure_threshold"]) > int(production["account_sync_failure_threshold"])
    assert int(paper["execution_degradation_min_samples"]) > int(production["execution_degradation_min_samples"])
    assert float(paper["execution_degradation_block_edge_bps"]) < float(production["execution_degradation_block_edge_bps"])
    assert float(paper["execution_degradation_throttle_scale"]) > float(production["execution_degradation_throttle_scale"])
