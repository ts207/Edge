...
def test_environment_specific_services_pin_distinct_configs_and_snapshot_paths() -> None:
    ...
    production = Path("deploy/systemd/edge-live-engine-production.service").read_text(encoding="utf-8")
    ...
    assert "ExecStart=/opt/edge/.venv/bin/edge-live-engine --config ${EDGE_LIVE_CONFIG} --snapshot_path ${EDGE_LIVE_SNAPSHOT_PATH}" in paper
    assert "ExecStart=/opt/edge/.venv/bin/edge-live-engine --config ${EDGE_LIVE_CONFIG} --snapshot_path ${EDGE_LIVE_SNAPSHOT_PATH}" in production
...
def test_environment_example_files_pin_distinct_credentials_and_runtime_paths() -> None:
    ...
    production = Path("deploy/env/edge-live-engine-production.env.example").read_text(encoding="utf-8")
