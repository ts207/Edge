...
class TestAblationBH:
    def test_output_has_condition_q_value_column(self):
        ...
        assert "condition_q_value" in result.columns, "condition_q_value column missing from ablation output"
    ...
    def test_bh_uses_alpha_010_threshold_for_condition_significance(self):
        ...
        for _, row in result.iterrows():
            ...
            if row["condition_q_value"] <= 0.10:
                assert row["is_condition_discovery"] is True or row["is_condition_discovery"] == True
            else:
                assert row["is_condition_discovery"] is False or row["is_condition_discovery"] == False

    def test_output_has_condition_p_value_raw_column(self):
        group = _make_group([
            {"candidate_id": "c1", "condition_key": "all", "expectancy": 0.001, "n_events": 100, "p_value": 1.0},
            {"candidate_id": "c2", "condition_key": "cond_a", "expectancy": 0.002, "n_events": 40, "p_value": 0.03},
        ])
        result = calculate_lift(group)
