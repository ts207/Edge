...
def test_future_row_injection_affects_output():
    ...
    res1 = audited_merge_asof(
        left, right, left_on="enter_ts", right_on="timestamp", 
        feature_name="test", stale_threshold_seconds=3600, symbol="BTC", run_id="test"
    )
    ...
    res2 = audited_merge_asof(
        left, right_mod, left_on="enter_ts", right_on="timestamp", 
        feature_name="test", stale_threshold_seconds=3600, symbol="BTC", run_id="test"
    )
...
def test_timestamp_forward_shift_failure():
    ...
    res_normal = audited_merge_asof(
        left, right, left_on="enter_ts", right_on="timestamp", 
        feature_name="feature", stale_threshold_seconds=3600, symbol="BTC", run_id="test"
    )
    res_shifted = audited_merge_asof(
        left_shifted, right, left_on="enter_ts", right_on="timestamp", 
        feature_name="feature", stale_threshold_seconds=3600, symbol="BTC", run_id="test"
    )
...
def test_future_timestamp_join_hard_fail():
    ...
    res = audited_merge_asof(
        left, right, left_on="enter_ts", right_on="timestamp", 
        direction="backward", feature_name="test", stale_threshold_seconds=3600, symbol="BTC", run_id="test"
    )
