...
def test_research_smoke(tmp_path: Path):
    ...
    if 'gate_phase2_final' in df.columns:
        ...
        assert 'is_discovery' in df.columns, "is_discovery column missing — rejection path not observable"
        assert (~df['is_discovery']).any(), "All candidates marked is_discovery=True — rejection path not exercised"
