...
def test_debug_features():
    ...
    if len(df.index[raw_mask]) == 0:
        # Investigate why
        recent_cluster = features["forced_flow"].rolling(48, min_periods=1).max().fillna(0).astype(bool)
        cluster_direction = np.sign(
            features["ret_1"].where(features["forced_flow"], 0.0).rolling(48, min_periods=1).sum().shift(1)
        ).replace(0.0, np.nan).ffill().fillna(0.0)
        
        rebound_window = 12
        ff_cooldown = ~(
            features["forced_flow"].rolling(rebound_window, min_periods=1).max().shift(1).fillna(0).astype(bool)
        )
...
def test_forced_flow_cooldown_impact():
    ...
    # Shorter cooldown
    events_short = detector.detect(df, symbol="BTCUSDT", rebound_window_bars=4, threshold_window=100)
