            ...
            if isinstance(node.func, ast.Attribute) and node.func.attr == 'quantile':
                ...
                try:
                    ...
                    if not has_prior_shift:
                        errors.append(f"{self.filename}:{node.lineno} - Unshifted quantile call found: {expr_str}")
                except Exception:
                    pass
                ...
                class ShiftVisitor(ast.NodeVisitor):
                    ...
                    def visit_Call(self, node):
                        if isinstance(node.func, ast.Attribute) and node.func.attr == 'shift':
                            ...
                            if isinstance(caller, ast.Call) and isinstance(caller.func, ast.Attribute) and caller.func.attr == 'quantile':
                        ...
                        if isinstance(node.func, ast.Attribute) and node.func.attr == 'quantile':
                            if node not in shift_visitor.shifted_quantile_nodes:
                                ...
                                if '.shift(' not in expr_str:
                                    errors.append(f"{filepath.name}:{node.lineno} - Unshifted quantile call: {expr_str}")
                        self.generic_visit(node)
