...
def registry_root(tmp_path):
    ...
    (root / "events.yaml").write_text(yaml.dump({
        "version": 1,
        "events": {
            "VOL_SPIKE": {
                "family": "volatility",
    ...
    (root / "templates.yaml").write_text(yaml.dump({
        "version": 1,
        "templates": {
    ...
    (root / "contexts.yaml").write_text(yaml.dump({
        "version": 1,
        "context_dimensions": {
    ...
    (root / "search_limits.yaml").write_text(yaml.dump({
        "version": 1,
        "limits": {"max_events_per_run": 5, "max_templates_per_run": 5}
    }))
    
    (root / "detectors.yaml").write_text(yaml.dump({
        "version": 1,
        "detector_ownership": {
...
def test_validation_failure_invalid_event(registry_root, tmp_path):
    ...
    config_path.write_text(yaml.dump({
        "program_id": "bad",
        "run_mode": "research",
        "instrument_scope": {"instrument_classes": ["crypto"], "symbols": [], "timeframe": "5m", "start": "", "end": ""},
        "trigger_space": {"allowed_trigger_types": ["EVENT"], "events": {"include": ["NON_EXISTENT"]}},
        "templates": {"include": ["continuation"]},
        "evaluation": {"horizons_bars": [1], "directions": ["long"], "entry_lags": [0]},
        "contexts": {"include": {}},
        "search_control": {"max_hypotheses_total": 10, "max_hypotheses_per_template": 5, "max_hypotheses_per_event_family": 5},
        "promotion": {"enabled": True}
    }))
    
    with pytest.raises(ValueError, match="Event ID 'NON_EXISTENT' is not in the authoritative registry"):
