...
def test_run_all_forwards_template_selection_to_event_level_phase2(monkeypatch, tmp_path):
    ...
    assert rc == 0
    phase2_stages = [args for stage, args in captured if stage.startswith("phase2_conditional_hypotheses__")]
...
def test_run_all_template_only_selection_fans_out_across_event_chain(monkeypatch, tmp_path):
    ...
    assert rc == 0
    phase2_stage_names = [stage for stage, _ in captured if stage.startswith("phase2_conditional_hypotheses__")]
...
def test_run_all_writes_research_comparison_report_when_baseline_is_configured(monkeypatch, tmp_path):
    captured: list[tuple[str, str, str]] = []
    report_path = tmp_path / "data" / "reports" / "research_comparison" / "candidate_run" / "vs_baseline_run" / "research_run_comparison.json"

    def fake_run_stage(stage, script, base_args, run_id, **kwargs) -> bool:
        return True

    def fake_write_run_comparison_report(*, data_root, baseline_run_id, candidate_run_id, out_dir=None, report_out=None, summary_out=None, thresholds=None, drift_mode="warn"):
        captured.append((str(data_root), baseline_run_id, candidate_run_id))
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(json.dumps({"baseline_run_id": baseline_run_id, "candidate_run_id": candidate_run_id}), encoding="utf-8")
        return report_path
...
def test_run_all_enforce_mode_fails_when_research_comparison_exceeds_threshold(monkeypatch, tmp_path):
    report_path = tmp_path / "data" / "reports" / "research_comparison" / "candidate_run" / "vs_baseline_run" / "research_run_comparison.json"
...
def test_run_all_wires_candidate_promotion_into_compiler(monkeypatch, tmp_path):
    ...
    assert stage_names.index("update_edge_registry") < stage_names.index("generate_recommendations_checklist")
...
def test_run_all_explicitly_disabled_candidate_promotion_is_not_reenabled_by_legacy_alias(monkeypatch, tmp_path):
    captured: list[tuple[str, list[str]]] = []
