...
def registry_root(tmp_path):
    ...
    (reg_dir / "events.yaml").write_text(yaml.dump({
        "events": {
            "E1": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": True, "requires_features": ["f1"]},
            "E2": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": True, "requires_features": ["f2"]}
        }
    }))
    
    (reg_dir / "states.yaml").write_text(yaml.dump({
        "states": {
            "S1": {"enabled": True, "instrument_classes": ["crypto"]}
        }
    }))
    
    (reg_dir / "features.yaml").write_text(yaml.dump({
        "features": {
    ...
    (reg_dir / "templates.yaml").write_text(yaml.dump({
        "templates": {
            "tpl": {"enabled": True, "supports_trigger_types": ["EVENT", "STATE", "SEQUENCE", "FEATURE_PREDICATE", "INTERACTION", "TRANSITION"]}
        }
    }))
    
    (reg_dir / "contexts.yaml").write_text(yaml.dump({"context_dimensions": {}}))
    (reg_dir / "search_limits.yaml").write_text(yaml.dump({"limits": {"max_events_per_run": 100, "max_templates_per_run": 100, "max_horizons_per_run": 10, "max_directions_per_run": 10, "max_sequence_length": 5}}))
    (reg_dir / "detectors.yaml").write_text(yaml.dump({"detector_ownership": {"E1": "D1", "E2": "D2"}}))
...
def _make_config(tmp_path, **overrides):
    config = {
        "program_id": "test",
        "run_mode": "research",
        "instrument_scope": {"instrument_classes": ["crypto"], "symbols": ["BTCUSDT"], "timeframe": "1m", "start": "2024-01-01", "end": "2024-01-02"},
        ...
        "contexts": {"include": {}},
        "search_control": {"max_hypotheses_total": 1000, "max_hypotheses_per_template": 1000, "max_hypotheses_per_event_family": 1000},
...
def test_expand_and_resolve_sequence(registry_root, tmp_path):
    conf = _make_config(tmp_path, trigger_space={
...
def test_expand_and_resolve_state_and_transition(registry_root, tmp_path):
    conf = _make_config(tmp_path, trigger_space={
        ...
        "states": {"include": ["S1"]},
        "transitions": {"include": [{"from_state": "S1", "to_state": "S1"}]} # Simplified self-transition for test
    })
    plan = build_experiment_plan(conf, registry_root)
...
def test_expand_and_resolve_feature_predicate(registry_root, tmp_path):
    conf = _make_config(tmp_path, trigger_space={
        ...
        "feature_predicates": {"include": [{"feature": "p1", "operator": ">", "threshold": 0.5}]}
    })
    plan = build_experiment_plan(conf, registry_root)
    assert len(plan.hypotheses) == 1
