...
def registry_root(tmp_path):
    ...
    (reg_dir / "events.yaml").write_text(yaml.dump({
        "events": {
            "VOL_SPIKE": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": True},
            "LIQ_GAP": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": True},
            "NOT_SEQ": {"enabled": True, "instrument_classes": ["crypto"], "sequence_eligible": False}
        }
    }))
    
    (reg_dir / "states.yaml").write_text(yaml.dump({
        "states": {
            "HIGH_VOL": {"enabled": True, "instrument_classes": ["crypto"]}
        }
    }))
    ...
    (reg_dir / "templates.yaml").write_text(yaml.dump({
        "templates": {
            "continuation": {"enabled": True, "supports_trigger_types": ["EVENT", "STATE", "SEQUENCE", "FEATURE_PREDICATE", "INTERACTION"]},
    ...
    (reg_dir / "search_limits.yaml").write_text(yaml.dump({
        "limits": {
...
def test_validate_template_compatibility(registry_root, tmp_path):
    ...
    with pytest.raises(ValueError, match="Template 'event_only' does not support trigger type 'STATE'"):
        build_experiment_plan(conf, registry_root)

def test_validate_sequence_eligibility(registry_root, tmp_path):
    conf = _make_config(tmp_path,
        trigger_space={"allowed_trigger_types": ["SEQUENCE"], "sequences": {"include": [["VOL_SPIKE", "NOT_SEQ"]]}}
    )
...
def test_validate_feature_predicate(registry_root, tmp_path):
    conf = _make_config(tmp_path,
        trigger_space={"allowed_trigger_types": ["FEATURE_PREDICATE"], "feature_predicates": {"include": [{"feature": "rsi", "operator": "=="}]}}
    )
...
def test_validate_instrument_mismatch(registry_root, tmp_path):
    ...
    with pytest.raises(ValueError, match="Event 'VOL_SPIKE' is not allowed for instrument class 'equities'"):
        build_experiment_plan(conf, registry_root)
